...
impl Front {
    pub fn load(ui_dir: &std::path::Path) -> Result<Self, String> {
        let chef = ui_dir
            .with_file_name(runity::dialogue::DIR)
            .join("chef.ron");
    ...
    pub fn ping(&mut self, ping: crate::state::Ping) {
        ...
        self.floaters
            .push(runity::glam::Vec3::from_array(ping.at), "!", colour);
    }
        ...
        if let Some(was) = self.last_score {
            ...
            if change > 0 {
                self.floaters.push(
                    WON_AT,
                    format!("+{change}"),
                    runity::glam::Vec4::new(1.0, 0.84, 0.3, 1.0),
                );
            } else if change < 0 {
                self.floaters.push(
                    LOST_AT,
                    format!("−{}", -change),
                    runity::glam::Vec4::new(1.0, 0.35, 0.25, 1.0),
                );
            }
        }
        ...
        for (cook, index) in local_cooks(world, party) {
            ...
            if actions.pressed(input, "ping") {
                if let Ok(t) = world.get::<&runity::Transform>(cook) {
                    let at =
                        t.position + crate::state::facing(&t) * 1.0 + runity::glam::Vec3::Y * 1.3;
                    party.publish(
                        crate::state::PING,
...
pub fn ring(world: &mut World) {
    for (moving, animates) in
        world.query_mut::<(&mut runity::motion::Moving, &runity::motion::Animates)>()
    {
    ...
    for (mark, emitting) in
        world.query_mut::<(&crate::components::Mark, &mut runity::particles::Emitting)>()
    {

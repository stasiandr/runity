...
impl Peer {
    ...
    fn in_scene(name: &str, party: Party, mut render: Option<Render>) -> Self {
        let scene =
            runity::project::data_file(env!("CARGO_MANIFEST_DIR"), &format!("scenes/{name}.ron"));
    ...
    fn frame(&mut self) {
        ...
        let mut make = |w: &mut World, prefab: &str, t: Transform| match render.as_mut() {
            Some(r) => live
                .spawn_prefab(prefab, t, None, w, &r.gpu, &mut r.renderer)
                .ok()
                .map(|i| i.root),
            None => live
                .spawn_prefab_headless(prefab, t, None, w)
                .ok()
                .map(|i| i.root),
        };
        let events = self
            .party
            .update(&mut self.world, &game_components(), 1.0 / 30.0, &mut make);
        session::act(&mut self.world, &self.party, &events);
        self.events.extend(events);
        session::frame(&mut self.world, &mut self.party, |w, prefab| {
            make(w, prefab, Transform::default())
        });
    ...
    fn boiling(&self) -> f32 {
        let stove = station_at(&self.world, STOVE);
        self.world
            .get::<&runity::world::Sounding>(stove)
            .map_or(0.0, |s| s.0.volume)
    }
...
fn station_at(world: &World, tile: (f32, f32)) -> Entity {
    world
        ...
        .iter()
        .find(|(_, t, _)| {
            (t.position.x - tile.0).abs() < 0.1 && (t.position.z - tile.1).abs() < 0.1
        })
        .map(|(e, ..)| e)
...
fn alone_a_player_plays_one_cook() {
    ...
    assert!(
        ...
        "the three nobody plays are out"
    );
...
fn the_kitchen_is_shut_until_the_host_opens_the_doors_for_everyone() {
    ...
    assert!(
        !round.open && round.orders.is_empty(),
        "nothing is ordered in the lobby"
    );
...
fn a_tomato_soup_goes_from_the_crate_to_the_window() {
    ...
    assert!(k.pot().done());
    assert!(
        k.mark_on(STOVE, "steam") && !k.mark_on(STOVE, "smoke"),
        "a done soup steams"
    );
    ...
    k.grab(0);
    assert!(
        noise.listen(&k.world).contains(&"ding"),
        "the bell for a soup served"
    );
...
fn raw_food_does_not_go_in_the_pot_and_mixed_soup_is_nobodys() {
    ...
    assert_eq!(
        k.round().note.map(|n| n.0).as_deref(),
        Some("Chop it first")
    );
...
fn a_pot_left_too_long_burns_and_is_scraped() {
    ...
    assert!(k.pot().burnt());
    assert!(
        k.mark_on(STOVE, "smoke") && k.mark_on(STOVE, "soup burnt"),
        "a burnt pot smokes"
    );
    k.plate_up(0);
...
fn orders_come_in_walk_out_and_the_round_ends_and_starts_again() {
    ...
    assert_eq!(
        k.held(0),
        Some(Thing::Food(Food::Tomato)),
        "nothing is done once time is up"
    );
    k.act(Act::Restart);
    let round = k.round();
...
fn owns(peer: &Peer, index: u32) -> bool {
    peer.world
        .get::<&runity::net::Owned>(peer.cook(index))
        .is_ok()
}
...
fn a_guest_is_seated_at_the_second_cook_and_walks_it() {
    ...
    assert_eq!(
        crate::front::local_cooks(&guest.world, &guest.party).len(),
        1
    );
...
fn what_a_guests_hands_do_the_host_does_and_everyone_sees() {
    ...
    until(&mut host, &mut guest, |_, g| {
        g.world.query::<&Round>().iter().next().is_some()
    });
...
fn the_screens_load_and_every_word_is_in_both_languages() {
    ...
    for id in [
        "host", "friends", "who", "music", "sfx", "language", "quit", "status", "best",
    ] {
        assert!(
            ids(&front.menu).contains(&id.to_string()),
            "menu has no `{id}`"
        );
    }
    for id in ["clock", "score", "players", "note", "keys", "leave"] {
        assert!(
            ids(&front.hud).contains(&id.to_string()),
            "hud has no `{id}`"
        );
    }
    for id in ["score", "served", "best", "again", "menu", "wait"] {
        assert!(
            ids(&front.results).contains(&id.to_string()),
            "results have no `{id}`"
        );
    }
    for id in ["title", "hint", "menu"] {
        assert!(
            ids(&front.lost).contains(&id.to_string()),
            "the host-lost card has no `{id}`"
        );
    }
    for id in ["players", "start", "wait", "invite", "leave"] {
        assert!(
            ids(&front.lobby).contains(&id.to_string()),
            "the lobby has no `{id}`"
        );
    }
...
fn the_board_on_the_wall_shows_the_orders_on_every_peer() {
    ...
    host.world.get::<&mut Round>(kitchen_e).unwrap().orders = vec![
        Order {
            dish: Dish::Soup(Food::Tomato),
            left: 30.0,
            total: 60.0,
        },
        Order {
            dish: Dish::Soup(Food::Onion),
            left: 60.0,
            total: 60.0,
        },
    ];
...
impl Render {
    ...
    fn new(width: u32, height: u32) -> Option<Self> {
        ...
        overlay
            .use_font(crate::front::FONT.to_vec())
            .expect("the kitchen's font");
        Some(Self {
            gpu,
            target,
            renderer,
...
fn the_particles_sprites_are_uploaded_with_the_kitchen() {
    ...
    for emitting in k.world.query::<&runity::particles::Emitting>().iter() {
        let map = emitting
            .material
            .and_then(|m| m.base_map)
            .expect("every effect here has a sprite");
        assert!(
            r.renderer.texture_for(map).is_some(),
            "{:?} has no sprite",
            emitting.emitter.material
        );
        sprites += 1;
    }
...
fn a_screenshot_of_the_menu_and_of_a_round_in_full_swing() {
    ...
    for (tile, food, cooked) in [
        (STOVE, Food::Tomato, COOK_SECONDS + 2.0),
        (
            (2.0, -3.0),
            Food::Onion,
            crate::components::pot::FIRE_SECONDS + 1.0,
        ),
    ] {
...
fn a_guest_who_leaves_takes_their_cook_out_and_the_host_keeps_it() {
    ...
    assert!(
        host.world.get::<&Seat>(hc).is_err(),
        "nobody plays the second cook"
    );
    ...
    assert!(host
        .events
        .iter()
        .any(|e| matches!(e, Event::Left { clean: true, .. })));
    ...
    let item = host
        ...
        .unwrap();
    assert!(
        host.world.get::<&At>(item).is_err(),
        "loose on the floor, for anyone"
    );
}
...
fn thrown_food_flies_and_lands_on_a_counter_in_a_pot_or_on_the_floor() {
    ...
    let (t, _) = k
        ...
        .unwrap();
    assert!(
        (t.position - start).length() > 1.0,
        "it went somewhere: {start} → {}",
        t.position
    );
    assert!(
    ...
    assert_eq!(
        k.held(0),
        Some(Thing::Food(Food::Tomato)),
        "back in hand from where it lay"
    );
}
...
fn what_a_guest_throws_flies_on_the_hosts_physics_and_everyone_sees_it() {
    ...
    guest.grab(1);
    until(&mut host, &mut guest, |_, g| {
        g.world.query::<&Item>().iter().count() == 1
    });
    // Turn round to the room, and throw.
    guest
        .world
        .get::<&mut Transform>(guest.cook(1))
        .unwrap()
        .rotation_deg
        .y = 0.0;
    guest.seconds(0.2);
    ...
    until(&mut host, &mut guest, |h, _| {
        h.world.query::<&Flying>().iter().count() == 1
    });
    let spot = |p: &Peer| {
        p.world
            .query::<(&Transform, &Item)>()
...
fn a_guest_sees_what_its_cook_holds_in_its_hands_at_once() {
    ...
    guest.grab(1);
    until(&mut host, &mut guest, |_, g| {
        g.world.query::<&crate::components::HeldBy>().iter().count() == 1
    });
...
fn the_camera_tour_loads() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let tour: runity::Tuned<runity::tour::Tour> =
        runity::Tuned::load(root.join("tuning/flyby.ron")).unwrap();
...
fn close_ups() {
    ...
    for i in 0..tour.shots.len() * 2 {
        ...
        let time = (i / 2) as f32 * leg
            + if i % 2 == 0 {
                tour.hold * 0.5
            } else {
                tour.hold + tour.travel * 0.5
            };
        let (at, look) = tour.at(time).unwrap();
        k.view = Some(runity::scene::View {
            position: at,
            target: look,
            fov_deg: 50.0,
        });
...
#[ignore = "pictures to look at"]
fn tour_frames() {
    let Some(render) = Render::new(960, 540) else {
        return;
    };
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let mut front = crate::front::Front::load(&root.join("ui")).unwrap();
...
fn a_salad_is_chopped_tomato_and_cabbage_on_a_plate() {
    ...
    assert_eq!(
        k.held(0),
        Some(Thing::Food(Food::Cabbage)),
        "raw, it stays in hand"
    );
    k.stand(0, BIN, -Vec3::X);
    k.grab(0);
...
fn a_served_plate_comes_back_dirty_and_is_washed_onto_the_rack() {
    let mut k = Peer::alone();
    let racks = |k: &Peer| -> u32 {
        [PLATES, RACK]
            .iter()
            .map(|t| {
                k.world
                    .get::<&Stack>(station_at(&k.world, *t))
                    .map_or(0, |s| s.0)
            })
            .sum()
    };
    ...
    assert_eq!(
        k.world.get::<&Stack>(station_at(&k.world, RACK)).unwrap().0,
        4
    );
}
...
fn recipes_close() {
    ...
    for (name, at, look) in [
        (
            "recipes_pans",
            Vec3::new(3.5, 2.4, -1.0),
            Vec3::new(3.5, 1.0, -3.0),
        ),
        (
            "recipes_island",
            Vec3::new(0.0, 2.4, 2.0),
            Vec3::new(0.0, 1.0, 0.0),
        ),
        (
            "recipes_sink",
            Vec3::new(0.5, 2.4, 1.0),
            Vec3::new(0.5, 1.0, 3.0),
        ),
        (
            "recipes_left",
            Vec3::new(-3.0, 2.6, -0.5),
...
fn a_guest_whose_host_closes_the_kitchen_is_told_and_offered_the_menu() {
    ...
    assert!(
        guest.events.contains(&Event::HostLost { quit: true }),
        "{:?}",
        guest.events
    );
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(
        words.contains(&"The host closed the kitchen") && words.contains(&"Back to the menu"),
        "{words:?}"
    );
}
...
fn a_burnt_pot_left_catches_fire_costs_points_and_is_scraped_out() {
    ...
    k.seconds(2.0);
    assert!(
        k.round().score <= before - 2 * crate::systems::cook::FIRE_COST,
        "{} from {before}",
        k.round().score
    );
    k.stand(0, STOVE, Vec3::Z);
    k.work(0, 0.2);
    assert!(
        !k.mark_on(STOVE, "fire") && k.pot().foods.is_empty(),
        "scraped out"
    );
...
fn burnt_meat_left_on_a_pan_catches_fire_and_is_scraped_off() {
    ...
    k.work(0, 0.2);
    assert!(
        !k.mark_on(PAN, "fire") && !k.mark_on(PAN, "smoke"),
        "the pan is clear"
    );
}
...
fn a_guest_says_they_are_ready_and_the_host_sees_it_in_the_lobby() {
    ...
    let words: Vec<&str> = ui.texts.iter().map(|t| t.text.as_str()).collect();
    assert!(
        words.contains(&"guest — ready") && words.contains(&"host"),
        "{words:?}"
    );
    assert!(
        words
            .iter()
            .any(|w| w.starts_with("Open the doors") && w.ends_with("(1/2)")),
        "{words:?}"
    );
}
...
fn in_rush_hour_the_island_rides_and_carries_what_is_on_it() {
    ...
    assert!(
        (at.z - start).abs() > 0.3,
        "the island has moved: {at} from {start}"
    );
    let plate = k
        .world
        .query::<(&Transform, &Item)>()
        ...
        .unwrap();
    assert!(
        (plate.z - at.z).abs() < 0.05 && (plate.x - at.x).abs() < 0.05,
        "and the plate on it: {plate} on {at}"
    );
}
...
fn the_host_takes_everyone_to_rush_hour() {
    ...
    let wanted = Event::SceneRequired {
        scene: "rush".into(),
    };
    until(&mut host, &mut guest, |h, g| {
        h.events.contains(&wanted) && g.events.contains(&wanted)
    });
}

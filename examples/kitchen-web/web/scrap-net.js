...
// written in by web/build.sh from SCRAP_TURN_URL; left as is, there is no
// relay and only devices STUN can join meet.
const TURN_URL = "__SCRAP_TURN_URL__";
...
const net = {
  ...
  _deliver(data) {
    ...
    this.wasm.scrap_net_receive(bytes);
    return bytes;
  },
  ...
  async invite() {
    ...
    try {
      ...
      window.scrapToast && window.scrapToast("Link copied: " + url);
    } catch (_) {

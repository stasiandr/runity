...
content/      everything a person makes
  kitchen/  the game's own folder
    maps/     levels
    core/     what everything else stands on; world.ron, the game's numbers
    food/, stations/, effects/  features: each thing's textures, materials and prefabs together
...
src/<feature>/  code by feature: `cooking/pot.rs` is the component `pot`, `cooking/cook.rs` a system
```

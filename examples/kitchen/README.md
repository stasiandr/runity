...
| **A camera tour** (`scrap::tour`), tuned from a file | the fly-through while players gather: the shots in `configs/flyby.ron` (saved while running, it changes at once), eased in and out in `main.rs`; the lens focuses on what it looks at |

...
pub fn act(world: &mut World, party: &Party, events: &[Event]) {
    ...
    for event in events {
        ...
        let allowed = match act {
            ...
            Act::Grab { cook } | Act::Throw { cook } | Act::Work { cook, .. } => state::cook(world, cook)
...
pub fn frame(world: &mut World, party: &mut Party, make: impl FnMut(&mut World, &str) -> Option<Entity>) {
...
pub fn spawns(world: &mut World, party: &mut Party, mut make: impl FnMut(&mut World, &str) -> Option<Entity>) {

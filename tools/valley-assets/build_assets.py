#!/usr/bin/env python3
"""Build `assets/valley/{models,textures}` from the CC0 source kits.
...
The sources are three Kenney kits, all CC0 (see `assets/valley/CREDITS.txt`):
...
Unzip them side by side and run:
    ...
    python3 tools/valley-assets/build_assets.py --kits /path/to/kits
...
def write_obj(path, entry, triangles):
    ...
    lines.append("# Generated by tools/valley-assets/build_assets.py; see")
    lines.append("# assets/valley/CREDITS.txt. Colors are cells of")
    lines.append("# assets/valley/textures/valley_atlas.png — no materials here.")
...
def write_credits(path, rows, atlas_name):
    lines = [
        "assets/valley — где что взято и на каких условиях.",
        "",
        ...
        "Конвертация: tools/valley-assets/build_assets.py.",
        "",
        "Наборы:",
    ]
...
def main():
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "assets" / "valley",

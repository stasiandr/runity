...
The sources are three Kenney kits, all CC0 (see `examples/valley/content/valley/CREDITS.txt`):
...
def write_obj(path, entry, triangles):
    ...
    lines.append("# examples/valley/content/valley/CREDITS.txt. Colors are cells of")
    lines.append("# examples/valley/content/valley/core/valley_atlas.png — no materials here.")
...
def write_credits(path, rows, atlas_name):
    lines = [
        "content/valley — где что взято и на каких условиях.",
        "",
...
# Where each model lives in the example: by feature, beside its materials
# (docs/layout.md).
FEATURES = {
    **{f"pine_{s}.obj": "nature/trees" for s in ("small", "medium", "large")},
    **{f"broadleaf_{s}.obj": "nature/trees" for s in ("small", "medium", "large")},
    **{f: "nature/plants" for f in ("grass_tuft.obj", "bush.obj", "berry_bush.obj", "tumbleweed.obj")},
    **{f: "nature/rocks" for f in ("boulder.obj", "boulder_small.obj")},
    "campfire.obj": "camp/campfire",
    **{f: "camp" for f in ("logs_loose.obj", "logs_bundle.obj", "axe.obj", "shelter.obj", "wall_unfinished.obj")},
    **{f: "settlers" for f in ("settler_stand.obj", "settler_carry.obj", "settler_walk.obj", "hands_first_person.obj")},
}
...
def main():
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "examples" / "valley" / "content" / "valley",
        help="the project's own folder: each model goes to its feature there (FEATURES), the atlas to core/",
    )
    args = parser.parse_args()

    textures_dir = args.out / "core"
    ...
    for entry in MANIFEST:
        ...
        models_dir = args.out / FEATURES.get(entry["file"], "")

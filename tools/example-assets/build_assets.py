#!/usr/bin/env python3
"""Build `assets/examples/{models,textures}` from the CC0 source kits.
...
The sources are three Kenney kits, all CC0 (see `assets/examples/CREDITS.txt`):
...
def write_obj(path, entry, triangles):
    ...
    lines.append("# assets/examples/CREDITS.txt. Colors are cells of")
    lines.append("# assets/examples/textures/valley_atlas.png — no materials here.")
...
def write_credits(path, rows, atlas_name):
    lines = [
        "assets/examples — где что взято и на каких условиях.",
        "",

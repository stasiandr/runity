...
* has a runity tab in the 3D view's sidebar (N): the game's components on
...
def _file_overrides():
    ...
    .rimport beside it, which the engine keeps (docs/blender.md). Read, not
    written: that choice lives in the project, not in the .blend."""
    path = bpy.data.filepath
    if not path:
        return {}
    try:
        with open(path + ".rimport") as f:
            text = f.read()
    except OSError:
        return {}
...
def _component_items(_self, _context):
    ...
    return items or [("", "(open the project in the runity editor)", "")]
...
class RUNITY_OT_add_component(bpy.types.Operator):
    """Add one of the game's components to the active object"""
    bl_idname = "runity.add_component"
...
class RUNITY_OT_remove_component(bpy.types.Operator):
    """Take this component off the active object"""
    bl_idname = "runity.remove_component"
...
class RUNITY_OT_toggle_collider(bpy.types.Operator):
    """Collide as its own shape in the game: a static body"""
    bl_idname = "runity.toggle_collider"
    bl_label = "Collider"
    bl_options = {"REGISTER", "UNDO"}
...
class RUNITY_OT_set_material(bpy.types.Operator):
    """Draw the active object in the game with a material of the project's"""
    bl_idname = "runity.set_material"
...
class RUNITY_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not path:
            ...
            layout.label(text="Not in a runity project", icon="ERROR")
        elif export.link_target() is None:
            layout.label(text="Editor closed: imported when it opens", icon="UNLINKED")
        else:
            layout.label(text="Linked to the runity editor", icon="LINKED")
        ...
        for name, fields in sorted(_components(obj).items()):
            ...
            header.operator("runity.remove_component", text="", icon="X").name = name
            if fields:
                for field in fields:
                    component.prop(obj, '["%s%s.%s"]' % (export.COMPONENT, name, field), text=field)
            else:
                component.prop(obj, '["%s%s"]' % (export.COMPONENT, name), text="")
        layout.operator_menu_enum("runity.add_component", "name", text="Add Component", icon="ADD")


CLASSES = (
    RUNITY_OT_add_component,
    RUNITY_OT_remove_component,
    RUNITY_OT_toggle_collider,
    RUNITY_OT_set_material,
    RUNITY_PT_panel,
)

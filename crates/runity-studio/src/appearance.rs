...
//! Support/runity/theme.ron` on macOS) and follows them into every
//! project, as Unity's skin does. It is a RON map, the same as a project's
//! `.runity/theme.ron`, with the preset named:
//!
//! ```ron
//! {
//!     "preset": "graphite",
...
//! ```
//!
//! **Which wins.** A project's `.runity/theme.ron`, when there is one, lays
//! its tokens over the person's, token by token. `.runity/` is not in git
...
/// `~/Library/Application Support/runity` on macOS, `%APPDATA%\runity` on
/// Windows, `$XDG_CONFIG_HOME/runity` or `~/.config/runity` elsewhere — or
/// `RUNITY_CONFIG_DIR`. None with no home folder to find.
...
impl Theme {
    ...
    pub fn poll(&mut self, project_root: Option<&Path>) -> (bool, Vec<String>) {
        ...
        let file = project_root.map(|r| r.join(".runity").join(FILE));
    ...
    pub fn show(&mut self, ui: &mut Ui, theme: &Theme) {
        ...
        if set.is_empty() {
            ...
            ui.set_text(
                self.note,
                &format!(
                    "This project's .runity/theme.ron sets {} over your choice.",
                    set.join(", ")
                ),
            );
...
mod tests {
    ...
    fn temp(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("runity-appearance-{name}-{}", std::process::id()));
    ...
    fn the_project_file_wins_token_by_token() {
        ...
        std::fs::create_dir_all(project.join(".runity")).unwrap();

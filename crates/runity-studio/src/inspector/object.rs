...
//! word: the shape of the field's type ([`runity::shape::Shape`]). Every
    ...
    fn bare(&self, session: &Session, key: &str) -> String {
        match self.holds {
            ...
            Holds::Link => {
                ...
                runity::ron::to_string(&link).unwrap_or_else(|_| tree::quote(key))
            }
            Holds::Typed => {
                ...
                let inner = if key.is_empty() {
                    ...
                    runity::ron::to_string(&session.link_to(&self.kind, key)).unwrap_or_default()
                };
                format!("{type_name}({inner})")
            }
            Holds::AssetId => tree::quote(if key.is_empty() { "0" } else { key }),
            Holds::EntityRef => format!("{}({})", runity::EntityRef::NAME, tree::quote(key)),
...
impl Inspector {
    ...
    pub(super) fn shown(&self, session: &Session, target: &ObjectRef, text: &str) -> Shown {
        ...
        match target.holds {
                ...
                let missing = name.as_deref().is_some_and(|n| {
                    !n.is_empty()
                        && match target.kind.as_str() {
                            ...
                            kind => !session.link_exists(kind, &runity::AssetLink::named(n)),
                        }
                });
                found(name, missing)
            }
            Holds::Typed => {
                let link = runity::refs::links_in(text)
                    .into_iter()
                    .next()
                    .map(|(_, l)| l);
                let missing = link
            ...
            Holds::AssetId => {
                let id = id_text(text).and_then(|t| t.parse::<runity::asset::AssetId>().ok());

...
use scrap_physics::bodies::{Physics, Shape};
use scrap_physics::body::{Body, Collider};
...
pub struct Fracture {
    ...
    #[serde(skip_serializing_if = "Option::is_none", with = "scrap_core::defaults::plain")]
    pub at: Option<f32>,
...
impl FractureLine for scrap_core::EntityDesc {
    fn fracture(&self) -> Option<Fracture> {
        self.part()
    }
}

impl FractureLine for scrap_core::scene::Override {
    fn fracture(&self) -> Option<Fracture> {
        self.part()
    }
}
...
#[derive(Debug, Clone)]
pub struct Piece {
    pub vertices: Vec<scrap_geometry::mesh_asset::Vertex>,
    pub indices: Vec<u32>,
    /// What it broke off.
...
pub fn run_fracture(world: &mut hecs::World, seconds: f32, blows: &[Blow]) -> Broken {
    ...
    let mut due = Vec::new();
    for (entity, breakable, placed, replica) in world.query_mut::<(hecs::Entity, &mut Breakable, &WorldTransform, Option<&scrap_core::world::Replica>)>() {
        if breakable.broken {
            continue;
        }
        ...
        if replica.is_some() && breakable.fracture.net == scrap_core::netsim::NetMode::Event {
            continue;
        }
...
impl scrap_core::world::Dress for FractureDress {
...
pub fn gather_net(world: &hecs::World, entity: hecs::Entity) -> Option<Vec<u8>> {
    ...
    if breakable.fracture.net != scrap_core::netsim::NetMode::Event || world.get::<&scrap_core::world::Replica>(entity).is_ok() {
        return None;
    }

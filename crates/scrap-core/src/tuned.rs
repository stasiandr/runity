...
//! # let dir = std::env::temp_dir().join("scrap-tuned-doc");
//! # std::fs::create_dir_all(&dir).unwrap();
//! # std::fs::write(dir.join("player.ron"), "(jump: 1.2, speed: 4.0)").unwrap();
//! let mut player = scrap_core::Tuned::<Player>::load(dir.join("player.ron")).unwrap();
...
mod tests {
    ...
    fn a_saved_number_reaches_the_game_and_a_bad_save_keeps_the_last_good_one() {
        let dir = std::env::temp_dir().join("scrap-tuned");

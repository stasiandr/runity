...
impl RopeLine for scrap_core::EntityDesc {
    fn rope(&self) -> Option<Rope> {
        self.part()
    }
}

impl RopeLine for scrap_core::scene::Override {
    fn rope(&self) -> Option<Rope> {
        self.part()
    }
}
...
pub fn run_ropes(world: &mut hecs::World, seconds: f32, obstacles: &Obstacles) {
    use scrap_core::netsim::NetMode;
    use scrap_core::world::Replica;
    let one_way = scrap_core::netsim::link_delay(world);
...
pub fn gather_net(world: &hecs::World, entity: hecs::Entity) -> Option<Vec<u8>> {
    ...
    if state.rope.net != scrap_core::netsim::NetMode::Full || world.get::<&scrap_core::world::Replica>(entity).is_ok() {
        return None;
    }
...
/// The soft module's dresser for ropes ([`scrap_core::world::Dress`]): a
/// line's `rope`, hung afresh when it changes.
pub struct RopeDress;

impl scrap_core::world::Dress for RopeDress {
    ...
    fn dress(
        ...
        match line.rope() {
            Some(rope) => {
                ...
                match rope.end.0.filter(|_| rope.net == scrap_core::netsim::NetMode::Full) {
                    Some(id) => {
...
mod tests {
    ...
    fn a_rope_tied_to_another_entity_follows_it_and_the_tube_is_whole() {
        ...
        let id: scrap_core::EntityId = "00000000000000aa".parse().unwrap();
        let post = world.spawn((
            WorldTransform(Mat4::from_translation(Vec3::new(5.0, 2.0, 0.0))),
            scrap_core::world::SceneId(id),
        ));

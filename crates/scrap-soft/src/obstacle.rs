...
pub struct Sheet {
    ...
    pub high: Vec3,
    cell: f32,
    cells: scrap_core::hash::FastMap<[i32; 3], Vec<u32>>,
}
...
impl Sheet {
    pub fn new(triangles: Vec<[Vec3; 3]>, thick: f32) -> Self {
        ...
        let mut cells: scrap_core::hash::FastMap<[i32; 3], Vec<u32>> = Default::default();
            ...
            for y in c[1] - r..=c[1] + r {
                for z in c[2] - r..=c[2] + r {
                    ...
                    for &i in here {
                        ...
                        let q = scrap_geometry::sdf::closest_on_triangle(p, a, b, cc);

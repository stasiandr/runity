...
//! [`scrap_geometry::field`]) — or as its drops (`Drops`), a small ball at
...
impl FluidLine for scrap_core::EntityDesc {
    fn fluid(&self) -> Option<Fluid> {
        self.part()
    }
}

impl FluidLine for scrap_core::scene::Override {
    fn fluid(&self) -> Option<Fluid> {
        self.part()
    }
}
...
pub struct FluidState {
    ...
    h: f32,
    rest: f32,
    grid: scrap_core::hash::FastMap<(i32, i32, i32), Vec<u32>>,
...
impl FluidState {
    ...
    fn find_neighbours(&mut self) {
        ...
        scrap_core::jobs::for_each_indexed_mut(&mut self.neighbours, 128, |i, list| {
            let p = x[i];
            let (cx, cy, cz) = key(p);
            list.clear();
            for dz in -1..=1 {
    ...
    fn step_pbf(&mut self, obstacles: &[Obstacle]) {
        ...
        for _ in 0..SUBSTEPS {
            ...
            for _ in 0..ITERATIONS {
                ...
                let found: Vec<(f32, f32)> = scrap_core::jobs::map_range(n, 128, |i| {
                ...
                scrap_core::jobs::for_each_indexed_mut(&mut moves, 128, |i, m| {
                    let p = x[i];
    ...
    fn viscosity(&mut self) {
        ...
        let (x, neighbours) = (&self.particles.x, &self.neighbours);
        scrap_core::jobs::for_each_indexed_mut(&mut self.particles.v, 128, |i, vi| {
            let p = x[i];
    ...
    fn step_sph(&mut self, obstacles: &[Obstacle]) {
        ...
        for _ in 0..SUBSTEPS {
            ...
            // across the cores.
            let (x, neighbours) = (&self.particles.x, &self.neighbours);
            scrap_core::jobs::for_each_indexed_mut(&mut self.density, 128, |i, density| {
                let p = x[i];
            ...
            scrap_core::jobs::for_each_indexed_mut(&mut self.particles.v, 128, |i, v| {
                let p = x[i];
    ...
    pub fn surface(&self, placed: Mat4) -> (Vec<Vertex>, Vec<u32>) {
        ...
        let mut field = scrap_geometry::field::Field::new(dims, origin, cell);
        for p in &self.particles.x {
...
impl scrap_core::world::Dress for FluidDress {

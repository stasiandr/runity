...
fn contact_pass(bodies: &mut [Body], sheets: &[Option<Vec<[u32; 3]>>], friction: f32, befores: &[Vec<Vec3>], touching: &[bool]) -> usize {
    ...
    let mut grid: scrap_core::hash::FastMap<(i32, i32, i32), Vec<Entry>> = Default::default();
    ...
    let found = scrap_core::jobs::map(&runs, 1, |run| {
    ...
    let sheet_runs: Vec<(usize, &[[u32; 3]])> = sheets
        ...
        .collect();
    let bodies_seen: &[Body] = bodies;
    let found = scrap_core::jobs::map(&sheet_runs, 1, |&(s, run)| {
        let mut out: Vec<(u32, u32, Vec3)> = Vec::new();
...
fn sheet_run(
    ...
    s: usize,
    tris: &[[u32; 3]],
    grid: &scrap_core::hash::FastMap<(i32, i32, i32), Vec<Entry>>,
    reach: f32,
    befores: &[Vec<Vec3>],
    thick: f32,
    most: f32,

...
fn modified(path: &Path) -> Option<SystemTime> {
    scrap_core::files::modified(path)
}
...
impl Actions {
    ...
    fn lay_over(&mut self, path: &Path) -> Result<(), String> {
        let Ok(text) = scrap_core::files::read_to_string(path) else {
            return Ok(());
        };
...
mod tests {
    ...
    fn a_player_rebinds_a_key_and_it_is_theirs_over_the_project() {
        use crate::input::InputEvent;
        let dir = std::env::temp_dir().join("scrap-rebind");
    ...
    fn a_rebinding_on_disk_takes_effect_and_a_broken_one_keeps_the_old() {
        let dir = std::env::temp_dir().join("scrap-actions");

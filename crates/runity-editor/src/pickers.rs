...
use runity::mesh_asset::TextureLibrary;
#[allow(unused_imports)]
use runity::prelude::*;
use runity::scene::EntityDesc;
use runity::EntityId;
    ...
    pub fn asset_ids_of_kind(&self, kind: &str) -> Vec<(String, AssetId)> {
        let mut out: Vec<(String, AssetId)> = match kind {
            "texture" => self
                ...
                .as_ref()
                .map(|l| {
                    l.names_of(runity::asset::TEXTURE)
                        .filter_map(|n| {
            ...
            "shader" => self
                .project
                .as_ref()
                .and_then(|p| std::fs::read_dir(p.root().join(runity::project::SHADERS)).ok())
                .into_iter()
                .flatten()
                .flatten()
                .filter_map(|e| {
                    ...
                    let id = runity::asset::shader_id(&name);
                    Some((name, id))
                })
                .collect(),
            _ => Vec::new(),

...
//! rest of the library. The runity plugin in an open Blender finds the
...
//!
//! A message is `RUNITYLK`, a `u32` kind (1 a scene, 2 placements), a
...
pub(crate) enum Message {
    ...
    Imported {
        source: PathBuf,
        result: Result<runity::AssetId, String>,
    },
...
impl Link {
    pub fn start(project: runity::Project) -> std::io::Result<Self> {
...
fn handle(project: &runity::Project, bytes: &[u8]) -> Option<Message> {
    ...
    match kind {
        1 => Some(Message::Imported {
            result: runity_import::blend::import_stream(project, &source, payload)
                .map_err(|e| format!("{e:#}")),
            source,
        }),
        2 => {
            let moves: Value = serde_json::from_slice(payload).ok()?;
            let scale = runity_import::ImportSettings::load(runity_import::sidecar_for(&source))
            ...
            Some(Message::Moved(
                moves
                    ...
                    .filter_map(|m| {
                        ...
                        let mut t = runity_import::blend::placement(

...
//! global one, sized to the cores or `RUNITY_JOBS`): a call costs a few
...
//! with `RUNITY_JOBS=1`, or on the web, where the core runs on one thread
    ...
    *WORKERS.get_or_init(|| {
        ...
        if n > 1 {
            ...
            let _ = rayon::ThreadPoolBuilder::new()
                .num_threads(n)
                .thread_name(|i| format!("runity-job-{i}"))
                .build_global();
        }
        n
    })

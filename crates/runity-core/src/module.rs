...
//!
//! A project lists its modules in `runity.ron` (`modules`); the engine
//! knows what each official one is by its manifest, and `runity check`
...
pub struct Manifest {
    /// What a project's `runity.ron` names it by: `physics`.
    pub name: String,
    pub version: String,
    /// The engine versions it fits: `0.1` for any 0.1.x.
    pub engine: String,
    /// One line on what it is, for `runity new` and the editor to show.
    #[serde(default)]
    pub what: String,
    ...
    for name in listed {
        let Some(module) = known.iter().find(|m| &m.name == name) else {
            ...
            let hint = crate::spelling::closest(name, names.iter().copied())
                ...
                .unwrap_or_default();
            out.push(format!("runity.ron lists module `{name}`, which the engine does not have{hint}"));
            continue;
        };

...
//!
//! Off unless asked ([`crate::Renderer::profile_gpu`], or
...
pub(crate) fn render(label: &'static str) -> Option<wgpu::RenderPassTimestampWrites<'static>> {
...
pub(crate) fn compute(label: &'static str) -> Option<wgpu::ComputePassTimestampWrites<'static>> {
    let (set, at) = next(label)?;
...
impl GpuTimer {
    ...
    pub(crate) fn begin(&mut self, gpu: &crate::Gpu) {
        ...
        if self.mapped.swap(false, Ordering::AcqRel) {
            if let Some(labels) = self.waiting.take() {
                let ticks: Vec<u64> = match self.read.slice(..).get_mapped_range() {
                    Ok(view) => bytemuck::cast_slice::<u8, u64>(&view).to_vec(),
                    Err(_) => Vec::new(),
                };
                self.read.unmap();
    ...
    pub(crate) fn frame_ms(&self) -> Option<f32> {
        self.last_frame
    }

    /// Each pass's average time, milliseconds.
    pub(crate) fn times(&self) -> Vec<(String, f32)> {

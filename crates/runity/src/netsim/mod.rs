...
pub fn register(components: &mut Components) {
    #[cfg(feature = "soft")]
    components
        .register_state("soft.rope", runity_soft::rope::gather_net, runity_soft::rope::take_net)
        .register_state("soft.cloth", runity_soft::cloth::gather_net, runity_soft::cloth::take_net);
    #[cfg(feature = "character")]
    components.register_state("character.ragdoll", runity_character::ragdoll::gather_net, runity_character::ragdoll::take_net);
    #[cfg(feature = "destruction")]
    components
        .register_state("destruct.fracture", runity_destruction::fracture::gather_net, runity_destruction::fracture::take_net)
        .register_state("destruct.dents", runity_destruction::dents::gather_net, runity_destruction::dents::take_net);
    let _ = components;
}
...
pub fn own_pieces(world: &mut World, pieces: &[(hecs::Entity, glam::Vec3)]) {
    use runity_destruction::{Breakable, Piece};
...
pub fn pull_bodies(world: &mut World, physics: &mut crate::physics::PhysicsWorld) {
    ...
    for (entity, state, placed) in world.query::<(hecs::Entity, &RopeState, &WorldTransform)>().iter() {
        if state.rope.ends != runity_soft::Ends::Both {
            continue;
        }

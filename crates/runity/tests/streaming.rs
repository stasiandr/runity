...
use runity::streaming::{StreamEvent, Streamed, Streamer};
use runity::{Gpu, OffscreenTarget, Prefabs, Renderer, Scene};
...
fn a_region_comes_in_as_the_camera_nears_it_and_goes_as_it_leaves() {
    ...
    let dir = std::env::temp_dir().join(format!("runity-stream-{}", std::process::id()));
    ...
    .unwrap();
    let world_scene: Scene = runity::ron::from_str(
        r#"(
    entities: [
    ...
    runity::spawn_scene(&world_scene, &mut world, |_| None);
    runity::world::apply_hierarchy(&mut world);
    let streamed = |world: &runity::hecs::World| world.query::<&Streamed>().iter().count();

...
use runity::material::RenderFace;
...
fn card_over_floor(gpu: &Gpu, post: runity::post::PostProcess) -> (usize, usize) {
...
fn a_cut_out_card_over_the_prepass_depth_keeps_its_holes() {
    ...
    let (red, white) = card_over_floor(&gpu, runity::post::PostProcess::default());
    ...
    let (plain_red, plain_white) = card_over_floor(&gpu, runity::post::PostProcess::OFF);
    assert!(

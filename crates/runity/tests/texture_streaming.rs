...
use runity::asset::{AssetId, TextureAsset, TextureLevel};
use runity::glam::{Mat4, Vec3};
use runity::render::{Camera, Draw, Frame};
...
fn a_texture_keeps_the_levels_the_screen_needs() {
    ...
    let bytes = runity::asset::to_bytes(&asset, runity::asset::TEXTURE).unwrap();
    let archived = runity::asset::view::<TextureAsset>(&bytes).unwrap();

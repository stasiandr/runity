...
fn the_prepass_runs_for_what_reads_its_depth_and_not_otherwise() {
    ...
    let graph = |ambient_occlusion: runity::ssao::AmbientOcclusion, post: runity::post::PostProcess| {
        let mut renderer = Renderer::new(&gpu, &target);
        let cube = renderer.upload_mesh_owned(&gpu, &builtin::cube(1.0));
        let frame = Frame {
            post,
            ambient_occlusion,
            shadows: runity::render::ShadowSettings {
                contact: 0.0,
                ..Default::default()
            },
            camera: Camera {
    ...
    let full = graph(Default::default(), runity::post::PostProcess::default());
    assert!(full.runs("prepass") && full.runs("ssao") && full.runs("taa"), "{}", full.dot());
    assert!(full.problems(&["shadow map", "rays"]).is_empty());
    let bare = graph(runity::ssao::AmbientOcclusion::OFF, runity::post::PostProcess::OFF);

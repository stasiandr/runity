...
use runity::upscale::{DynamicResolution, Method, Upscaling, Used};
use runity::{builtin, Gpu, Material, OffscreenTarget, Renderer};

const SIZE: u32 = 192;

fn scene(renderer: &mut Renderer, gpu: &Gpu, post: runity::post::PostProcess) -> Frame {
...
fn post(upscaling: Upscaling) -> runity::post::PostProcess {
    runity::post::PostProcess {
        upscaling,
        // A still picture: the eye already used to it.
        auto_exposure: runity::exposure::AutoExposure::OFF,
        ..runity::post::PostProcess::default()
    }
}

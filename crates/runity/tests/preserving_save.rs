...
use runity::{EntityDesc, Prefabs, Scene};
...
fn copy(relative: &str, name: &str) -> (PathBuf, String) {
    ...
    let dir = std::env::temp_dir().join(format!("runity-preserve-{name}"));
...
fn a_new_entity_is_added_and_a_removed_one_goes_leaving_the_rest() {
    ...
    runity::edit::remove(&mut scene, far);
    runity::edit::add(
        &mut scene,
        None,
        EntityDesc {
...
fn a_first_save_writes_missing_ids_into_the_lines_as_they_are() {
    let dir = std::env::temp_dir().join("runity-preserve-ids");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();

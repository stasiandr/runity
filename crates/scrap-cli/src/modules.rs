//! A project's modules (docs/modules.md): `scrap.ron` lists them, and the
//! engine's cargo features in the game's `Cargo.toml` follow from the list
//! — written by `scrap modules sync`, held to it by `scrap check`.

use anyhow::{bail, Context, Result};
use scrap::modules::{self, Manifest};
use scrap::Project;

/// The modules a project lists, with what they stand on; `None` when its
/// `scrap.ron` lists none.
...
/// counted when it does not turn them off; `None` without a `scrap`
/// dependency on one line.
pub fn cargo_features(cargo: &str) -> Option<Vec<String>> {
    let line = scrap_line(cargo)?;
...
/// The features a `scrap` line names, as written.
...
/// `cargo` with its `scrap` line building exactly `features` of the
/// modules, and whatever else it asked for.
pub fn with_features(cargo: &str, features: &[String]) -> Result<String> {
    let line = scrap_line(cargo).context("Cargo.toml has no `scrap = { ... }` line")?;
    let open = line.find('{').context("the scrap line is not an inline table")?;
    let close = line.rfind('}').context("the scrap line is not an inline table")?;
    ...
    let new = format!(
        "scrap = {{ {}, default-features = false, features = [{}] }}",
        kept.join(", "),
        list.join(", ")
    );
...
pub fn problems(project: &Project) -> Vec<String> {
    ...
    if have != want {
        out.push(format!(
            "Cargo.toml builds the engine with [{}], scrap.ron's modules want [{}] — `scrap modules sync` writes it",
            have.join(", "),
            want.join(", ")
        ));
    }
...
/// Write the game's `Cargo.toml` from `scrap.ron`'s modules; what it
/// builds with now.
pub fn sync(project: &Project) -> Result<Vec<String>> {
    let listed = &project.manifest().modules;
    if listed.is_empty() {
        bail!("scrap.ron lists no modules: add `modules: [...]` first (`scrap modules` shows them)");
    }
    let known = modules::official();
...
fn scrap_line(cargo: &str) -> Option<&str> {
    cargo
        .lines()
        .find(|l| l.trim_start().starts_with("scrap =") || l.trim_start().starts_with("scrap="))
}
...
mod tests {
    ...
    const KITCHEN: &str = "[dependencies]\nanyhow = \"1\"\nruntity = { path = \"../../crates/scrap\", features = [\"desktop-shell\", \"audio\"] }\n";

    #[test]
    fn the_features_a_cargo_line_builds_count_the_defaults_it_keeps() {
        let cargo = KITCHEN.replace("runtity", "scrap");
        assert_eq!(
        ...
        assert!(
            written.contains("scrap = { path = \"../../crates/scrap\", default-features = false, features = [\"audio\", \"ray-tracing\"] }"),
            "{written}"
        );

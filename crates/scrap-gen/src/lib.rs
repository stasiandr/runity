...
pub fn drafts(project: &scrap::Project) -> Vec<Draft> {
...
impl Generator {
    ...
    pub fn start(&mut self, session: &mut Session, request: Request) -> Result<u64, String> {
        ...
        scrap::project::valid_name(&request.name)?;
...
fn apply(
    ...
    let cache = project.root().join(".scrap").join("gen");

...
//! page's script — `web/scrap-net.js` in a game's web build). A page has
//! no sockets, so the bytes go out through `scrapNet.send` and come back
//! through [`scrap_net_receive`]; how the channels are found and opened
...
#[wasm_bindgen]
extern "C" {
    /// The page's `scrapNet.send(to, bytes)`: to endpoint `to` (a guest
    /// sends everything to the host, whatever `to` says).
    #[wasm_bindgen(js_namespace = scrapNet, js_name = send)]
    fn page_send(to: u32, bytes: &[u8]);
}

/// A datagram came in over one of the page's channels, sender first.
#[wasm_bindgen]
pub fn scrap_net_receive(bytes: &[u8]) {
    if bytes.len() < 4 {
        return;
    }

...
//!
//! **Started from the editor or the command line**: `SCRAP_NET` is
//! `host:ADDRESS` or `join:ADDRESS`, `SCRAP_PLAYER` the name to join
//! with, and `SCRAP_LINK` how bad to make the link (`poor`, `awful`,
//! `latency=80,loss=3`); [`Party::from_env`] reads them. Without
//! `SCRAP_NET` the game plays alone.
...
impl Party {
    ...
    /// The party `SCRAP_NET`, `SCRAP_PLAYER` and `SCRAP_LINK` describe,
    ...
    pub fn update(
        ...
        scrap_core::netsim::set_session_time(world, self.server_time());
        scrap_core::netsim::set_link_delay(world, self.sync.one_way_ticks);
...
mod tests {
    ...
    fn the_words_that_start_a_party_are_checked() {
        ...
        for (net, link, says) in [
            ...
            ("join:127.0.0.1:9", "bad", "SCRAP_LINK"),
        ] {

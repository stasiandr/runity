...
use runity_core::world::{Parent, WorldTransform};
...
impl JiggleLine for runity_core::EntityDesc {
    fn jiggle(&self) -> Option<Jiggle> {
        self.part()
    }
}

impl JiggleLine for runity_core::scene::Override {
    fn jiggle(&self) -> Option<Jiggle> {
        self.part()
    }
}
...
pub fn run_jiggle(world: &mut hecs::World, seconds: f32) {
    ...
    for (_, e) in &order {
        if let Ok(own) = world.get::<&runity_core::scene::Transform>(*e) {
...
impl runity_core::world::Dress for JiggleDress {

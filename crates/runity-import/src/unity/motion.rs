...
//! (`runity::motion`): position, rotation and scale curves by the path of
//! the thing they move, and the float curves runity has a place for —
...
use runity::motion::{Motion, Property, Track};
use yaml_rust2::Yaml;
...
fn from_yaml(b: &Yaml, report: &mut Report) -> Motion {
    ...
    // Unity is left-handed, runity right-handed: z mirrored, as a scene's

...
pub fn build(
    ...
    let mut texture = |index: usize,
     ...
     -> Result<Option<AssetId>> {
        ...
        written.push((id, runity::asset::to_bytes(&asset, runity::asset::TEXTURE)?));
    ...
    for md in &data.materials {
        ...
        if md.metallic_map.is_some() || md.roughness_map.is_some() || md.occlusion_map.is_some() {
            ...
            if let Some(asset) = mask(&data.images, md, id, format!("{stem}/{own} mask")) {
                ...
                written.push((
                    asset.id,
                    runity::asset::to_bytes(&asset, runity::asset::TEXTURE)?,
                ));
            }
        }
        ...
        written.push((id, runity::asset::to_bytes(&asset, runity::asset::MATERIAL)?));
    ...
    for mesh in data.meshes {
        ...
        for (j, mut primitive) in mesh.primitives.into_iter().enumerate() {
            ...
            written.push((id, runity::asset::to_bytes(&asset, runity::asset::MESH)?));

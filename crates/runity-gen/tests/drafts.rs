...
use runity_gen::{drafts, Generator, Input, Output, Provider, Request};
...
fn project(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("runity-gen-{name}"));
...
fn a_draft_takes_the_greybox_place_in_one_undo_step() {
    ...
    assert!(
        matches!(&desc.material, MaterialRef::Inline(_)),

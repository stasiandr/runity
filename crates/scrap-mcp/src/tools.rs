...
fn entity_fields() -> Value {
    json!({
        ...
        "particles": { "type": "string", "description": "RON: particles given off along its up — (rate: 30.0, life: 0.8, speed: 2.0, spread_deg: 20.0, size: 0.06, gravity: -1.0, color: (1.0, 0.6, 0.2)) — sparks, dust, spray; `gpu: true, graph: \"name\"` runs shaders/<name>.vfx.ron (docs/shadergraph.md); or None" },
...
pub fn list() -> Vec<Value> {
    ...
    vec![
        ...
        tool("start_game", "Play with the game's own code: save the open scene and run the project's game on it (cargo run, SCRAP_SCENE), drawing in the editor's Game view (the other players, when several, in windows of their own). What it prints goes to the console; read it with console. One game at a time — starting again stops the one running. players: 2 to 4 opens that many windows playing together on this machine (Unity's Multiplayer Play Mode): player 1 hosts, the others join once it is up, and console lines start with whose they are (\"player 2: ...\"). The count is remembered for the next start; players: 1 goes back to one. link makes the other players' connection bad on purpose — \"poor\", \"awful\", \"latency=80,jitter=10,loss=3,dup=1\" (round-trip ms, percent), or \"\" for a perfect one — also remembered.", json!({ "players": { "type": "integer", "minimum": 1, "maximum": 4 }, "link": { "type": "string" }, "fast_game": { "type": "boolean", "description": "Build the game's own code optimized (opt-level 1): slower to build after an edit, several times quicker to run. Remembered for the next start." }, "from_here": { "type": "boolean", "description": "Play from Here: the player starts on the ground the view looks at (render's eye and target), facing its way — the game moves the lines its scene marks player_start: true there (SCRAP_START). Refused when there is nothing to stand on." } }), &[]),
        ...
        tool("shader_graph", "A shader graph (shaders/<name>.graph.ron: a material's; <name>.vfx.ron: a GPU particle effect's; <name>.post.ron: over the whole picture, named by a scene's `fullscreen`; <name>.subgraph.ron) told in words: its properties, every node with its inputs, what it sets, whether it builds (in the compiler's words, as the renderer would build it) and its warnings. An empty name lists them. Nodes and their inputs: docs/shadergraph.md.", json!({ "name": { "type": "string", "description": "lava, embers.vfx, cracks.subgraph, or the file name" } }), &["name"]),
        tool("shader_graph_edit", "Change a shader graph in its file where the change goes, the comments and every other line as they were, and say whether it builds after. Give `node` and `input` with `value` to set a node's input; `part` (surface, vertex; spawn, update, output; output for a .post.ron) and `input` with `value` to set what the graph sets; `node` and `add` to add a node; `node` and `remove: true` to take one out with its line. Values as RON: \"noise.x\" names a node (with its components), 0.5, (1.0, 0.2, 0.0).", json!({
            "name": { "type": "string" },
            "node": { "type": "string" },
            "part": { "type": "string" },
            "input": { "type": "string" },
...
pub fn call(server: &mut Server, name: &str, args: &Value) -> Answer {
    ...
    match name {
        ...
        "start_game" => {
            ...
            if let Some(fast) = args.get("fast_game").and_then(Value::as_bool) {
                session.set_fast_game(fast);
            }
            let from_here = args
                .get("from_here")
                .and_then(Value::as_bool)

...
use scrap::refs::AssetRef;
use scrap::scene::MaterialRef;
use scrap::{Library, Prefabs, Project, Scene};
...
fn two_sources_with_one_name_in_two_folders_are_two_assets() {
    let root = std::env::temp_dir().join(format!("scrap-same-name-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&root);
    let project = scrap::Project::create(&root, "same-name").unwrap();
    let square = "v 0 0 0\nv 1 0 0\nv 1 0 1\nf 1 2 3\n";
    let bigger = "v 0 0 0\nv 4 0 0\nv 4 0 4\nf 1 2 3\n";
    ...
    let a =
        scrap_import::built_for(&root.join("assets/rocks/rock.obj"), &project.library()).unwrap();
    let b =
        scrap_import::built_for(&root.join("assets/cliffs/rock.obj"), &project.library()).unwrap();
...
fn prefabs_scenes_graphs_and_screens_get_an_id_that_follows_a_move() {
    ...
    let id_of = |path: &std::path::Path| {
        scrap_import::ImportSettings::load(scrap_import::sidecar_for(path))
            .unwrap()
            .asset_id()
    };
    let door = id_of(&root.join("prefabs/door.prefab"));
    let sidecar = std::fs::read_to_string(root.join("prefabs/door.prefab.scrimport")).unwrap();
    assert!(
    ...
    for scene in std::fs::read_dir(project.scenes()).unwrap().flatten() {
        ...
        if path.extension().is_some_and(|e| e == "ron") {
            assert!(
                scrap_import::sidecar_for(&path).is_file(),
                "{}",
                path.display()
            );
        }
    ...
    assert!(
        scrap_import::identify(&project).is_empty(),
    ...
    .unwrap();
    let moved = scrap_import::identify(&project);
    assert!(
        matches!(moved[0].change, scrap_import::Change::Moved { .. }),
        "{moved:?}"
    );
    assert_eq!(id_of(&root.join("prefabs/props/door.prefab")), door);
    assert!(!root.join("prefabs/door.prefab.scrimport").exists());
    let _ = std::fs::remove_dir_all(&root);
}

...
//! name — and this is the test that the whole path works: a `.scrmat` written in
...
fn write_material(dir: &Path, library: &Path, stem: &str, body: &str) {
    ...
    scrap_import::import_file(
        &source,
        library,
        scrap_import::ImportSettings::for_source(format!("{stem}.scrmat")),
    )
    .unwrap();
}
...
fn surface_of(scene: &Scene, library: &Library) -> Material {
    ...
    let missing = scrap::spawn_scene_with(
        scene,
        &mut world,
    ...
    let surface = world.query::<&scrap::Surface>().iter().map(|s| s.0).next();
...
fn editing_the_hex_changes_what_the_scene_draws() {
    ...
    touch_forward(&scrap_import::built_for(&source, &project.library()).unwrap());

    let changed = library.reload_changed();
    assert_eq!(changed.len(), 1);
    assert_eq!(changed[0].kind, scrap::asset::MATERIAL);
...
fn a_colour_that_is_not_a_colour_says_so_instead_of_importing_black() {
    ...
    let err = scrap_import::import_file(
        &source,
        dir.join("library"),
        scrap_import::ImportSettings::for_source("broken.scrmat"),
    )
    .unwrap_err();
    ...
    assert!(
        !dir.join("library").join("broken.scrmat.scrasset").exists(),
...
fn the_example_palette_stands_in_for_the_builtins() {
    ...
    for entry in std::fs::read_dir(&sources).expect("the example palette") {
        ...
        scrap_import::import_to(
            ...
            &library_dir,
            &sidecar,
            scrap_import::ImportSettings::for_source(relative),
        )
    ...
    for name in scrap::material::builtin::NAMES {
        ...
        let builtin = scrap::material::builtin::by_name(name).unwrap();
    ...
    assert!(scrap::material::builtin::by_name("moss").is_none());
    ...
    let named = scene
        .flatten()
        .iter()
        .filter(|(e, _)| !matches!(&e.material_ref(), scrap::scene::MaterialRef::Inline(_)))
        .count();
...
fn a_library_from_an_older_format_is_rebuilt_by_sync() {
    ...
    assert!(scrap_import::sync(&project).is_empty(), "nothing to do");
    ...
    assert_eq!(done[0].change, scrap_import::Change::Outdated);
    assert!(done[0].result.is_ok(), "{:?}", done[0].result);
    assert!(scrap::asset::is_current(&asset));
}
...
fn an_instance_is_its_parent_with_what_it_says_changed_and_follows_the_parent() {
    let root = std::env::temp_dir().join(format!("scrap-instances-{}", std::process::id()));
...
(parent: "stone", smoothness: 0.8)"#,
    ...
    assert!(
        changed.iter().any(|r| r.source.ends_with("wet_stone.scrmat")),
        "{changed:?}"
    );
    ...
    let odd = said
        .iter()
        .find(|r| r.source.ends_with("odd.scrmat"))
        .unwrap();
    assert!(
        odd.result
            .as_ref()
...
fn a_material_saying_which_faces_and_what_surface_builds() {
    ...
    assert_eq!(glass.render_face, scrap::material::RenderFace::Both);
}
...
fn a_materials_textures_for_its_shader_survive_the_rmat_and_the_asset() {
    ...
    .unwrap();
    let written = scrap_import::material_source(&source).unwrap();
    ...
    let error = format!(
        "{:#}",
        scrap_import::import_into(&project, &source, None).unwrap_err()
    );

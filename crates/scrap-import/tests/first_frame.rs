...
use scrap::render::{Camera, Draw, FogSettings, Frame, Lighting};
...
fn a_source_model_becomes_a_frame() {
    ...
    let out_dir = std::env::temp_dir().join("scrap-first-frame");
    ...
    scrap_import::import_to(
        &source,
        &library_dir,
        &out_dir.join("conifer.obj.scrimport"),
        settings,
    )
    ...
    let lighting = Lighting {
        ...
        sun_direction: scrap::glam::Vec3::new(-1.0, -0.25, 0.0).normalize(),
        ..Lighting::default()
    };
    let fog = FogSettings {
        start: 20.0,
    ...
    let frame = Frame {
        ...
        camera: Camera {
            position: scrap::glam::Vec3::new(0.0, height * 0.55, height * 1.5),
            target: scrap::glam::Vec3::new(0.0, height * 0.5, 0.0),
            ..Camera::default()
        },
        lighting,
        fog,
        ...
        shadows: scrap::ShadowSettings::OFF,
        ...
        draws: vec![
            Draw {
                ...
                material: scrap::material::builtin::NEEDLE,
                pose: None,
            },
            Draw {
                mesh: handle,
                ...
                transform: scrap::glam::Mat4::from_translation(scrap::glam::Vec3::new(
                    9.0, 0.0, -38.0,
                )),
                texture: scrap::TextureHandle::WHITE,
                material: scrap::material::builtin::NEEDLE,
...
fn an_asset_from_an_older_format_does_not_reach_the_gpu() {
    ...
    let dir = std::env::temp_dir().join("scrap-first-frame-bad");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let mut bytes = scrap::asset::to_bytes(
        &MeshAsset {
            id: scrap::AssetId::from_source("x", 0),
            name: "x".into(),
            vertices: Vec::new(),
            indices: Vec::new(),
            submeshes: Vec::new(),

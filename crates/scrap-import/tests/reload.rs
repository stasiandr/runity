...
fn project(name: &str) -> (Project, PathBuf) {
    let root = std::env::temp_dir().join(format!("scrap-reload-{name}"));
...
fn a_fresh_clone_with_no_library_builds_all_of_it() {
    ...
    write(&root.join("materials/clay.scrmat"), r##"(color: "#8a5a3c")"##);
    import_into(&project, &root.join("materials/clay.scrmat"), None).unwrap();
...
fn a_source_outside_the_project_is_imported_with_a_warning() {
    ...
    let elsewhere = std::env::temp_dir().join("scrap-reload-elsewhere");
    ...
    let sidecar = project.assets().join("wedge.obj.scrimport");
    let settings = ImportSettings::load(&sidecar).unwrap();
...
fn a_library_re_reads_only_what_changed() {
    ...
    touch_forward(&one);
    sync(&project);
    touch_forward(&scrap_import::built_for(&one, &project.library()).unwrap());

    let changed = loaded.reload_changed();
    assert_eq!(changed.len(), 1, "one asset, not both");
    assert_eq!(
        changed[0].path,
        scrap_import::built_for(&one, &project.library()).unwrap()
    );
...
fn a_terrain_is_described_imported_as_a_mesh_and_rebuilt_when_its_seed_changes() {
    ...
    let source = root.join("assets/hills.scrterrain");
...
fn a_painted_heightmap_shapes_the_terrain_and_repainting_it_rebuilds() {
    ...
    let before =
        asset::read(scrap_import::built_for(&source, &project.library()).unwrap()).unwrap();

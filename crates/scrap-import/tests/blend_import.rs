//! A `.blend` in `assets/` imports itself through Blender and the scrap
...
//! and `scrap.<name>` properties are components.
//!
...
rock["scrap.door"] = "(open_angle: 90.0)"
for i in range(2):
    twin = rock.copy()
...
fn plugin() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tools/blender/scrap")
}
...
fn a_blend_imports_as_a_level_and_a_kit_through_blender() {
    ...
    let root = std::env::temp_dir().join("scrap-blend-import");
    ...
    assert!(
        (p - scrap::glam::Vec3::new(1.0, 3.0, -2.0)).length() < 1e-4,
        "{p}"
    );
    ...
    assert!(
        ...
        "scrap.door is a component"
    );
    assert_eq!(first.material_ref(), MaterialRef::Named("yard/moss".into()));
    ...
    let expanded = scrap::instantiate(
        &scrap::ron::from_str(r#"(entities: [(name: "yard", prefab: "yard")])"#).unwrap(),
        &prefabs,
    );
...
/// The scrap tab's buttons, driven as a person would: a component added
...
/// three back.
#[test]
fn the_scrap_panel_adds_components_a_field_at_a_time() {
    let Some(blender) = scrap_import::blend::blender() else {
        eprintln!("no Blender on this machine: skipped");
        return;
    };
    let root = std::env::temp_dir().join("scrap-blend-panel");
    let _ = std::fs::remove_dir_all(&root);
...
assert bpy.ops.scrap.add_component(name="door") == {{"FINISHED"}}
assert bpy.ops.scrap.add_component(name="tag") == {{"FINISHED"}}
assert isinstance(door["scrap.door.open_angle"], float)
assert door["scrap.door.locked"] is False or door["scrap.door.locked"] == 0
door["scrap.door.open_angle"] = 90.0
door["scrap.door.table"] = "chest"
bpy.ops.scrap.toggle_collider()
bpy.ops.scrap.set_material(material="stone")
...
"#,
    ...
    assert_eq!(door.material_ref(), MaterialRef::Named("stone".into()));
}
...
fn the_plugin_installs_into_blender_turned_on() {
    ...
    let home = std::env::temp_dir().join("scrap-blender-user");
    ...
    let folder = scrap_import::blend::install(&blender).unwrap();
    ...
    let check = Command::new(&blender)
        ...
        .arg(
            "import bpy, addon_utils\n\
             assert addon_utils.check('bl_ext.user_default.scrap')[1], 'not on'\n\
             assert any(h.__name__ == '_saved' for h in bpy.app.handlers.save_post), 'no handler'\n\
             assert hasattr(bpy.types, 'SCRAP_PT_panel'), 'no panel'\n",
        )

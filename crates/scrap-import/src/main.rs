//! `scrap-import` — the command line behind the editor's drag-and-drop.
//!
//! ```text
//! scrap-import --sync [PROJECT]          build the project's library from its sources
//! scrap-import <source>... [options]     import into the source's project
//! scrap-import <source>... --library DIR import into a library of your choosing
//! ```
//!
//! `--sync` is what a fresh clone runs: the library is derived and not
//! committed, so it is built from `assets/`, `materials/` and the `.scrimport`
...
use scrap_import::{import_file, import_into, sync, Change, ImportSettings};
...
fn main() -> Result<()> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "scrap-import --sync [PROJECT]\n\
                     scrap-import <source>... [--library DIR] [--scale F] \
                     ...
                     sources: .gltf .glb .obj .png .jpg .tga .bmp .wav .scrmat"
                );
                return Ok(());
            }
    ...
    if sync_requested {
        ...
        let project = scrap::Project::find(&at).map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    for source in &sources {
        let out = match &library {
            ...
            None => {
                let project = scrap::Project::find(source)

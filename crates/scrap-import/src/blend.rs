...
//! background with the scrap plugin's exporter (`tools/blender/scrap/`),
...
//! `.scrasset` is written here, by the engine's own code: the plugin never
//! learns the engine's formats (postulate 7, one truth per subsystem).
//!
//! The stream: `SCRAP_BL`, a `u32` version, a `u32` length and that much
...
pub const EXPORTER: &str = include_str!("../../../tools/blender/scrap/export.py");

/// The add-on itself, as Blender installs it: what [`install`] writes.
pub const ADDON: [(&str, &str); 3] = [
    (
        "__init__.py",
        include_str!("../../../tools/blender/scrap/__init__.py"),
    ),
    ("export.py", EXPORTER),
    (
        "blender_manifest.toml",
        include_str!("../../../tools/blender/scrap/blender_manifest.toml"),
    ),
];

/// Install the scrap add-on into this Blender and turn it on: an
...
files = json.loads(os.environ["SCRAP_ADDON_FILES"])
folder = os.path.join(bpy.utils.user_resource("EXTENSIONS", path="user_default", create=True), "scrap")
...
try:
    ...
    print("refresh:", e)
addon_utils.enable("bl_ext.user_default.scrap", default_set=True, persistent=True)
bpy.ops.wm.save_userpref()
print("SCRAP_INSTALLED " + folder)
"#;
    ...
    match text
        .lines()
        .find_map(|l| l.strip_prefix("SCRAP_INSTALLED "))
    {
...
const MAGIC: &[u8; 8] = b"SCRAP_BL";

/// How long Blender gets to open a file and send it.
const TIMEOUT: Duration = Duration::from_secs(120);

/// Where Blender is: `SCRAP_BLENDER`, then where macOS installs it, then
/// `blender` on the `PATH`.
pub fn blender() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("SCRAP_BLENDER") {
...
pub fn read(source: &Path) -> Result<SceneData> {
    let Some(blender) = blender() else {
        bail!(
            "Blender is needed to import a .blend and was not found — install it, or set SCRAP_BLENDER to its executable"
        );
    };
...
pub fn parse(bytes: &[u8]) -> Result<SceneData> {
    ...
    ensure!(take(8)? == MAGIC, "not a scrap stream");
    let version = u32::from_le_bytes(take(4)?.try_into()?);
    ensure!(
        version == VERSION,
        "the plugin speaks version {version} and this importer {VERSION}: update the scrap plugin in Blender"
    );

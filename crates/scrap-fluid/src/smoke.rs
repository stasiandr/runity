...
//! it burns (`scrap_render::volume::Smoke`).
...
impl SmokeLine for scrap_core::EntityDesc {
    fn smoke(&self) -> Option<Smoke> {
        self.part()
    }
}

impl SmokeLine for scrap_core::scene::Override {
    fn smoke(&self) -> Option<Smoke> {
        self.part()
    }
}
...
impl SmokeState {
    ...
    /// `scrap_render::smoke_gpu`) up to [`SmokeState::clock`].
    ...
    fn project(&mut self, cells: &[(usize, usize, usize)]) {
        ...
        for _ in 0..SWEEPS {
            ...
            let slice = nx * ny;
            scrap_core::jobs::for_each_chunk_mut(&mut next, slice, |first, cells| {
                let k = first / slice;
                if k == 0 || k + 1 >= nz {
                    return;
                }
...
fn each_awake_cell<T: Send>(field: &mut [T], n: [usize; 3], awake: &[bool], edge: usize, f: impl Fn(usize, usize, usize, usize, &mut T) + Sync) {
    ...
    let [bx, by, bz] = [nx / BLOCK, ny / BLOCK, nz / BLOCK];
    let slice = nx * ny;
    scrap_core::jobs::for_each_chunk_mut(field, slice, |first, cells| {
        let k = first / slice;
...
impl scrap_core::world::Dress for SmokeDress {

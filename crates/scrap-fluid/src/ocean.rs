...
impl OceanLine for scrap_core::EntityDesc {
    fn ocean(&self) -> Option<Ocean> {
        self.part()
    }
}

impl OceanLine for scrap_core::scene::Override {
    fn ocean(&self) -> Option<Ocean> {
        self.part()
    }
}
...
pub fn run_oceans(world: &mut hecs::World, seconds: f32) {
    ...
    for (state, placed) in world.query_mut::<(&mut OceanState, &WorldTransform)>() {
        ...
        scrap_soft::net::keep_time(&mut state.time, clock);
...
impl scrap_core::world::Dress for OceanDress {

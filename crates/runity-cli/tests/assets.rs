//! `runity rename`, seen the way a reviewer sees it: in `git diff`.
//!
...
fn runity(root: &Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_runity"))
        .current_dir(root)
        .args(args)
        .output()
        .unwrap()
...
fn a_rename_is_a_move_and_one_line_per_use_in_the_diff() {
    ...
    let root = std::env::temp_dir().join("runity-cli-rename");
    ...
    .unwrap();
    assert!(runity(&root, &["sync"]).status.success());
    git(&root, &["init", "-q", "-b", "main"]);
    git(&root, &["config", "user.email", "test@runity"]);
    git(&root, &["config", "user.name", "test"]);
    git(&root, &["add", "-A"]);
    git(&root, &["commit", "-q", "-m", "start"]);

    let used = runity(&root, &["uses", "materials/clay.rmat"]);
    ...
    let out = runity(
        &root,
        &["rename", "materials/clay.rmat", "materials/terracotta.rmat"],
    );
    assert!(
        out.status.success(),
        "{}{}",
    ...
    assert!(
        lines.contains(&"R100\tmaterials/clay.rmat\tmaterials/terracotta.rmat"),
        "{status}"
    );
    // The sidecar's `source:` line changed, so git pairs it by similarity.
    assert!(
        lines.iter().any(|l| l.starts_with('R')
            && l.ends_with("\tmaterials/clay.rmat.rimport\tmaterials/terracotta.rmat.rimport")),
        "{status}"
    );

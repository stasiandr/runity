//! What `runity build` lays out: the game and its data, and no sources.

use runity::Project;

#[test]
fn a_build_carries_the_data_the_game_reads_and_leaves_the_sources_home() {
    let root = std::env::temp_dir().join("runity-build-package");
    ...
    let shipped = runity_cli::build::package(&project, &exe, &out).unwrap();
    ...
    runity_cli::build::package(&project, &exe, &out).unwrap();

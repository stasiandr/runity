//! Two branches edit one scene, and git merges them through `runity merge`.
//!
//! The whole path, with real git: a project, two branches, the driver
//! configured the way `runity git-setup` configures it (pointing at this
...
fn repository(name: &str) -> Option<(PathBuf, PathBuf)> {
    ...
    let root = std::env::temp_dir().join(format!("runity-merge-{name}"));
    ...
    ok(&root, &["config", "user.email", "test@runity"]);
    ok(&root, &["config", "user.name", "test"]);
    ok(&root, &["config", "merge.runity.name", "runity"]);
    let driver = format!("{} merge %O %A %B %P", env!("CARGO_BIN_EXE_runity"));
    ok(&root, &["config", "merge.runity.driver", &driver]);
    ok(&root, &["add", "-A"]);
...
fn git_setup_turns_the_driver_on_in_a_clone() {
    ...
    ok(&root, &["config", "--unset", "merge.runity.driver"]);
    let out = Command::new(env!("CARGO_BIN_EXE_runity"))
        .args(["git-setup"])
        .arg(&root)
        .output()
        .unwrap();

//! `runity new --template NAME`: a project of the engine's own — a folder
//! of `examples/` with a `runity.ron` — copied and handed over (DNA,
...
const DERIVED: [&str; 5] = ["target", "library", ".runity", "build", "Cargo.lock"];
...
/// The templates there are: every example with a `runity.ron`.
pub fn names() -> Vec<String> {
    let mut out: Vec<String> = std::fs::read_dir(examples())
        .into_iter()
        .flatten()
        .flatten()
        .filter(|e| e.path().join(runity::project::FILE).is_file())
...
pub fn create(template: &str, folder: &Path, name: &str, engine: &Engine) -> Result<Project> {
    ...
    if !from.join(runity::project::FILE).is_file() {
        bail!("no template `{template}` — there are: {}", names().join(", "));
    }
    if folder.join(runity::project::FILE).exists() {
        bail!("{} is already a project", folder.display());
    }
    copy(&from, folder)?;

    let manifest = folder.join(runity::project::FILE);
    ...
    if cargo.is_file() {
        ...
        let text = pointed(&text, &runity::project::crate_name(name), &engine.cargo_source(folder))
            .context("the template's Cargo.toml has no runity line with a path")?;
...
/// `runity.ron` with the project's `name` in place of the template's.
...
/// `Cargo.toml` with the package named `crate_name` and the `runity` line
...
fn pointed(text: &str, crate_name: &str, source: &str) -> Option<String> {
    ...
    for line in text.lines() {
        ...
        if line.trim_start().starts_with("runity =") {
...
mod tests {
    ...
    fn a_template_is_renamed_and_pointed_at_the_engine() {
        let cargo = "[package]\nname = \"kitchen\"\n\n[dependencies]\nruntity = { path = \"../../crates/runity\", features = [\"audio\"] }\n"
            .replace("runtity", "runity");
        let out = pointed(&cargo, "soup", "git = \"https://example.com/runity\"").unwrap();
        assert!(out.contains("name = \"soup\""), "{out}");
        assert!(
            out.contains("runity = { git = \"https://example.com/runity\", features = [\"audio\"] }"),
            "{out}"
        );

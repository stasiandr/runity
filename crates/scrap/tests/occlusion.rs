...
//! picture is the picture without culling.
...
fn what_stands_behind_a_wall_is_left_out_and_nothing_seen_is() {
    ...
    let shot = |culling: bool| {
        ...
        for i in 0..50 {
            ...
            draws.push(draw(cube, Mat4::from_translation(Vec3::new(x, 0.5, z)) * Mat4::from_scale(Vec3::splat(0.5))));
        }
        for i in 0..10 {
            let x = (i as f32 - 4.5) * 0.4;
            draws.push(draw(cube, Mat4::from_translation(Vec3::new(x, 0.15, 1.0)) * Mat4::from_scale(Vec3::splat(0.2))));
        }
        let frame = Frame {
            camera: Camera {

...
use scrap::asset;
use scrap::{builtin, Gpu, LiveScene, Model, OffscreenTarget, Project, Renderer, SceneId};
...
fn rock(project: &Project, size: f32) {
    ...
    mesh.name = "rock".into();
    mesh.id = scrap::AssetId::from_source("assets/rock.obj", 0);
    let bytes = asset::to_bytes(&mesh, scrap::asset::MESH).unwrap();
    write(&project.library().join("rock.obj.scrasset"), bytes);
}

fn entity(world: &hecs::World, id: &str) -> hecs::Entity {
    let id: scrap::EntityId = id.parse().unwrap();
    world
...
fn a_prefab_spawned_at_run_time_is_whole_and_outlives_a_reload() {
    ...
    let at = scrap::Transform {
        position: scrap::glam::Vec3::new(10.0, 0.0, 0.0),
    ...
    let ember = world
        .query::<(hecs::Entity, &Warmth, &scrap::world::WorldTransform)>()
        .iter()
...
fn a_link_in_a_prefab_spawned_at_run_time_finds_its_own_instances_part() {
    ...
    let at = |x: f32| scrap::Transform {
        position: scrap::glam::Vec3::new(x, 0.0, 0.0),
        ..scrap::Transform::default()
    };
    ...
    for (lamp, x) in [(one, 0.0), (two, 5.0)] {
        ...
        let placed = world.get::<&scrap::world::WorldTransform>(switch).unwrap().0.w_axis;
...
fn a_joint_in_a_prefab_spawned_at_run_time_holds_its_parts_together() {
    ...
    live.spawn_headless(&mut world);
    let at = scrap::Transform {
        position: scrap::glam::Vec3::new(0.0, 5.0, 0.0),
        ..scrap::Transform::default()
    };
    live.spawn_prefab_headless("lamp", at, None, &mut world).unwrap();
    let mut physics = scrap::PhysicsWorld::new(1.0 / 50.0);
    for _ in 0..100 {
        physics.run(&mut world);
    }
    let lowest = world
        .query::<(&scrap::world::Physics, &scrap::world::WorldTransform)>()
        .iter()
        .filter(|(p, _)| p.0 == scrap::scene::Body::Dynamic)
...
fn after_a_hot_patch_the_world_starts_again_under_new_types_keeping_what_a_save_keeps() {
    ...
    let mut before = scrap::Components::new();
    before
    ...
    world
        .get::<&mut scrap::Transform>(door)
        .unwrap()
        .position
        .x = 3.0;
    world.get::<&mut Tint>(door).unwrap().0 = 0.9;

    let mut after = scrap::Components::new();
    after
    ...
    assert_eq!(
        world.get::<&scrap::Transform>(door).unwrap().position.x,
        3.0
    );

...
use scrap::{EntityDesc, EntityId, MeshHandle, Scene, Transform};
...
fn level() -> Scene {
    ...
    for i in 0..PROPS {
        let part = |j: u64, model: &str| {
            ...
            .with(scrap::scene::MaterialRef::Named("stone".into()))
        };
...
fn reading_a_big_scene_file_is_within_budget() {
    ...
    let path = std::env::temp_dir().join("scrap-budget-level.ron");
...
fn a_one_field_edit_to_a_big_scene_reaches_the_world_within_budget() {
    ...
    after.entities[PROPS / 2].transform = Transform {
        position: scrap::glam::Vec3::new(0.0, 9.0, 0.0),
        ..Transform::default()
    };
    let mut world = scrap::hecs::World::new();
    scrap::spawn_scene(&before, &mut world, |_| Some(MeshHandle::TEST));
    ...
    let took = best(7, || {
        ...
        forward = !forward;
        let patched =
            scrap::patch_scene(from, to, &mut world, |_| Some(MeshHandle::TEST), |_| None);
...
fn building_a_frame_of_a_big_level_is_within_budget_and_steady() {
    ...
    let mut times = scrap::FrameTimes::new(60);
    for _ in 0..60 {
        let start = Instant::now();
        let frame = scrap::build_frame(
            &world,
            camera,
            scrap::scene_lighting(&scene.sun()),
            scrap::scene_fog(&scene.fog()),
        );

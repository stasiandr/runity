...
use scrap::ddgi::{IrradianceVolume, PlacedVolume};
...
fn slab(mesh: scrap::render::MeshHandle, centre: Vec3, size: Vec3, colour: [f32; 3]) -> Draw {
    Draw {
        mesh,
...
fn shot(gpu: &Gpu, target: &OffscreenTarget, draws: impl Fn(scrap::render::MeshHandle) -> Vec<Draw>, camera: Camera, volume: Option<PlacedVolume>, lighting: Lighting) -> [f32; 3] {

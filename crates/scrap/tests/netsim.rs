...
use scrap::glam::Vec3;
use scrap::net::wire::Conditions;
use scrap::netsim::bench::{ticks, Session, HZ};
use scrap::netsim::scenes::*;
use scrap::scene::{Scene, Transform};
use scrap::soft::{Rope, RopeKind, RopeState};
use scrap::{EntityId, EntityRef};
...
fn a_session_of_one_swings_as_the_game_alone() {
    ...
    let scene = pendulum(scrap::netsim::NetMode::Full);
    let mut alone = Session::new(&scene, 1, Conditions::GOOD, 1);
    let mut world = hecs::World::new();
    scrap::spawn_scene(&scene, &mut world, |_| Some(scrap::render::MeshHandle::TEST));
    scrap::world::apply_hierarchy(&mut world);
    let mut physics = scrap::physics::PhysicsWorld::new(1.0 / HZ);
    physics.sync_from_world(&mut world);
    let load = scrap::net::addressable(&world)[&EntityId::from_raw(LOAD)];
    for _ in 0..ticks(2.0) {
        alone.step();
        // The bench's step, but for the party.
        scrap::world::apply_hierarchy(&mut world);
        scrap::netsim::claim_approaching(&mut world, &physics);
        scrap::character::step(&mut world, &mut physics, 1.0 / HZ);
        physics.run(&mut world);
        scrap::world::apply_hierarchy(&mut world);
        scrap::netsim::anchor_ropes(&mut world, &mut physics);
        scrap::soft::step(&mut world, 1.0 / HZ);
        scrap::netsim::pull_bodies(&mut world, &mut physics);
        scrap::destruction::step(&mut world, &mut physics, 1.0 / HZ);
    }
...
fn tug_seen(peer: &scrap::netsim::bench::Peer) -> (f32, f32, f32) {
...
fn ends_apart(peer: &scrap::netsim::bench::Peer) -> f32 {
    peer.entity(PAWN_A)
...
fn tug_of_war(link: Conditions, seed: u64) -> Tug {
    ...
    const B: f32 = 700.0;
    let secs = 3.0;
    let scene = tug(scrap::netsim::NetMode::Full);
    let rest = APART * 1.03;
    ...
    for p in &mut session.peers {
        for id in [PAWN_A, PAWN_B] {
            ...
            let _ = p.world.insert_one(e, scrap::netsim::Pawn);
        }
    }
    assert!(session.join(), "everyone in: {:?}", session.problems());
...
/// `SCRAP_ACROSS` (`taut,damping,keep,gate,catch,catch_damping,slack`),
/// on two links and two seeds each, printed: what `scripts` sweep.
#[test]
#[ignore]
fn sweep_one() {
    let Ok(text) = std::env::var("SCRAP_ACROSS") else { return };
    let v: Vec<f32> = text.split(',').map(|x| x.trim().parse().unwrap()).collect();
    *scrap::netsim::ACROSS.write().unwrap() = scrap::netsim::Across {
        taut: v[0],
        damping: v[1],
        keep: v[2],
        gate: v[3],
    ...
    for (name, link) in [("poor", Conditions::POOR), ("bad", bad)] {
        let seeds: Vec<u64> = std::env::var("SCRAP_SEEDS").map(|s| s.split(',').map(|x| x.parse().unwrap()).collect()).unwrap_or(vec![3, 11]);
        for seed in seeds {
...
/// the body's owner.
fn run(peer: &mut scrap::netsim::bench::Peer, tick: usize) {
    let Some(e) = peer.entity(RUNNER) else { return };
    if peer.world.get::<&scrap::net::Owned>(e).is_err() {
        return;
    }
    ...
    if let Ok(mut t) = peer.world.get::<&mut Transform>(e) {
        ...
        t.set_rotation(scrap::glam::Quat::from_rotation_y(-angle));
    }
}

/// Where the cape's bottom middle and the tail's tip are, in the runner's
/// space, on a peer.
fn hanging(peer: &scrap::netsim::bench::Peer) -> (Vec3, Vec3) {
    let body = peer.entity(RUNNER).unwrap();
    let back = peer.world.get::<&scrap::world::WorldTransform>(body).unwrap().0.inverse();
    let cape = peer.entity(CAPE).and_then(|e| peer.world.get::<&scrap::soft::ClothState>(e).ok().map(|s| {
        let (w, h) = s.grid();
        s.points()[(h - 1) * w + w / 2]
    }));
...
fn capes(cape: scrap::netsim::NetMode, link: Conditions, seed: u64) -> ([f32; 2], [f32; 2], f32, f32) {
...
fn a_cape_and_a_tail_on_someone_elses_runner_do_not_shake_on_an_awful_link() {
    let (cape, tail, cape_unlike, tail_unlike) = capes(scrap::netsim::NetMode::Rough, Conditions::AWFUL, 5);
...
fn a_rough_cape_is_no_further_from_its_owners_than_a_local_one() {
    ...
    let local = mean(scrap::netsim::NetMode::Local);
    let rough = mean(scrap::netsim::NetMode::Rough);
...
fn pieces(peer: &scrap::netsim::bench::Peer) -> std::collections::BTreeMap<u64, Vec3> {
    peer.world
        .query::<(&scrap::destruction::Piece, &scrap::world::NetId, &Transform)>()
        .iter()
...
fn standing(peer: &scrap::netsim::bench::Peer) -> (f32, f32) {
    let e = peer.entity(PERSON).unwrap();
    let s = peer.world.get::<&scrap::character::RagdollState>(e).unwrap();
...
fn a_player_knocked_limp_sags_and_gets_up_alike_for_everyone() {
    ...
    let parts: Vec<u64> = {
        ...
        let e = p.entity(PERSON).unwrap();
        let s = p.world.get::<&scrap::character::RagdollState>(e).unwrap();
        s.parts.iter().map(|part| p.world.get::<&scrap::world::SceneId>(*part).unwrap().0.raw()).collect()
    };
    ...
    for t in 0..ticks(8.0) {
        session.step_with(|i, p| {
            ...
            if (ticks(0.17)..ticks(0.3)).contains(&t) && i == 1 {
                let e = p.entity(PERSON).unwrap();
                let parts = p.world.get::<&scrap::character::RagdollState>(e).unwrap().parts.clone();
...
fn shove(peer: &mut scrap::netsim::bench::Peer) {
        ...
        if t > ticks(0.4) {
            for p in &session.peers {
                for id in [CRATE_A, CRATE_B] {
                    let e = p.entity(id).unwrap();
                    if p.world.get::<&scrap::net::Owned>(e).is_ok() {
...
fn ten_chains_swinging_at_once_keep_within_the_budget_and_none_is_left_behind() {
    ...
    let one = pendulum(scrap::netsim::NetMode::Full);
...
fn traffic() {
    ...
    fn measure(name: &str, scene: Scene, pawns: &[u64], claims: &[u64], game: impl FnMut(usize, usize, &mut scrap::netsim::bench::Peer)) {
    ...
    fn measure_tagged(
        ...
        claims: &[u64],
        tag: Option<u64>,
        mut game: impl FnMut(usize, usize, &mut scrap::netsim::bench::Peer),
    ) {
    ...
    measure("chain", pendulum(scrap::netsim::NetMode::Full), &[], &[POST], |_, _, _| {});
    measure("tug", tug(scrap::netsim::NetMode::Full), &[PAWN_A, PAWN_B], &[PAWN_B], |_, _, p| {
    ...
    let run = |t: usize, _: usize, p: &mut scrap::netsim::bench::Peer| {
        let Some(e) = p.entity(RUNNER) else { return };
        if p.world.get::<&scrap::net::Owned>(e).is_err() {
            return;
        }
        let angle = t as f32 / HZ;
        if let Ok(mut tr) = p.world.get::<&mut Transform>(e) {
            tr.position = Vec3::new(angle.cos() * 3.0, 0.8, angle.sin() * 3.0);
            tr.set_rotation(scrap::glam::Quat::from_rotation_y(-angle));
        }
    };
    measure("runner", runner(scrap::netsim::NetMode::Rough), &[RUNNER], &[RUNNER], run);
    measure_tagged("tagged runner", runner(scrap::netsim::NetMode::Rough), &[RUNNER], &[RUNNER], Some(RUNNER), run);

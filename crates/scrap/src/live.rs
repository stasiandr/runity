...
//! `scrap sync` — turns a changed `.png` into a changed `.scrasset`,
//! and this picks the `.scrasset` up.
//!
...
pub struct LiveScene {
    ...
    /// this game (`SCRAP_STATE_FILE`); see [`LiveScene::report`].
...
pub const STATE_VAR: &str = "SCRAP_STATE_FILE";
...
impl LiveScene {
    ...
    /// `SCRAP_STATE_FILE`. Unity's Inspector during play, across a process
    ...
    /// Report to this file instead of the one `SCRAP_STATE_FILE` names;
...
mod tests {
    ...
    fn a_game_started_from_an_editor_reports_where_things_are() {
        let dir = std::env::temp_dir().join(format!("scrap-report-{}", std::process::id()));

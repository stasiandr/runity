...
use scrap::glam::Vec3;
use scrap::render::Camera;
use scrap::{builtin, Gpu, Library, MeshHandle, OffscreenTarget, Renderer, Scene};
...
fn render_shot(
    ...
    for (field, text) in shot.look {
        scrap::scene::set_look_field(&mut document, field, text)?;
    }
    ...
    let (prefabs, _) = project
        ...
        .unwrap_or_default();
    let mut scene = scrap::instantiate(&document, &prefabs).scene;
    ...
    scrap::terrain::upload_terrains(&mut world, gpu, &mut renderer);
    for problem in scrap::world::upload_material_maps(&world, library.as_ref(), gpu, &mut renderer)
    {
        eprintln!("{problem}");
    }
    ...
    let mut twin = if shot.compare {
        ...
        for (relief, model) in
            world.query_mut::<(&scrap::terrain::Relief, &scrap::world::Model)>()
        {
            let again = twin.upload_mesh_owned(gpu, &relief.terrain.mesh());
            assert_eq!(again, model.0, "the twin's terrain out of step");
        }
        for problem in scrap::world::upload_material_maps(&world, library.as_ref(), gpu, &mut twin)
        {
            eprintln!("{problem}");
        }
        Some(twin)
    ...
    scrap::physics::attach_scene_collision_meshes(&mut world, &scene, library.as_ref());
    let step = 1.0 / 60.0;
    let mut physics = scrap::PhysicsWorld::new(step);
    ...
    let base = scrap::scene_camera(&scene.view());
    ...
    for i in 0..warmup + frames {
        ...
        scrap::routes::run_routes(&mut world, dt * shot.speed);
        scrap::world::apply_hierarchy(&mut world);
        owed += dt * shot.speed;
        while owed >= step {
            scrap::fluid::float(&world, &mut physics);
            scrap::character::step(&mut world, &mut physics, step);
            physics.run(&mut world);
            scrap::fluid::step(&mut world, step);
            scrap::destruction::step(&mut world, &mut physics, step);
            owed -= step;
        }
        scrap::world::apply_hierarchy(&mut world);
        // What bends, hangs and flows, on its own fixed steps.
        scrap::soft::step(&mut world, dt * shot.speed);
        scrap::soft::show(&mut world, 0.0);
        scrap::destruction::show(&mut world, 0.0);
        scrap::fluid::show(&mut world, 0.0);
        scrap::character::crawl(&mut world, dt * shot.speed);
        scrap::character::show(&mut world, 0.0);
        scrap::footprints::run_footprints(&mut world, dt * shot.speed);
        scrap::particles::run_particles(&mut world, dt * shot.speed);
        ...
        let mut frame = scrap::world::scene_frame(&world, camera, &scene);
        ...
        let pixels = if let Some(twin) = twin.as_mut() {
            ...
            frame.ray_tracing = scrap::ray::RayTracing::default();

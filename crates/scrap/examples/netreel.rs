...
use scrap::netsim::bench::{ticks, Peer, Session, HZ};
use scrap::netsim::scenes::*;
use scrap::render::{Camera, Frame};
use scrap::scene::{Scene, Transform};
#[allow(unused_imports)]
use scrap::prelude::*;
use scrap::{builtin, Gpu, MeshHandle, OffscreenTarget, Renderer};
...
fn shots() -> Vec<Shot> {
    ...
    out.push(Shot {
        ...
        scene: pendulum(scrap::netsim::NetMode::Full),
        seconds: 5.0,
    ...
    out.push(Shot {
        ...
        scene: tug(scrap::netsim::NetMode::Full),
        seconds: 4.0,
    ...
    out.push(Shot {
        ...
        scene: runner(scrap::netsim::NetMode::Rough),
        seconds: 5.0,
    ...
    out.push(Shot {
        ...
        game: Box::new(|tick, peer, p| {
            if peer == 1 && (ticks(0.5)..ticks(0.63)).contains(&tick) {
                ...
                let parts = p.world.get::<&scrap::character::RagdollState>(e).map(|s| s.parts.clone()).unwrap_or_default();
                if parts.len() > 2 {
...
fn film(gpu: &Gpu, half: &OffscreenTarget, mut shot: Shot, file: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut meshes = |peer: usize, name: &scrap::AssetLink| {
        let name: &str = name;
    ...
    let lighting = scrap::scene_lighting(&shot.scene.sun());
    let camera = Camera { position: shot.camera.0, target: shot.camera.1, ..scrap::scene_camera(&shot.scene.view()) };
    let fog = scrap::render::FogSettings::default();
    ...
    for tick in 0..ticks(shot.seconds) {
        ...
        if tick % 2 == 0 {
            ...
            let pair = [0, 1].map(|i| {
                ...
                scrap::soft::show(&mut p.world, 0.0);
                scrap::destruction::show(&mut p.world, 0.0);
                scrap::character::show(&mut p.world, 0.0);
                scrap::build_frame(&p.world, camera, lighting.clone(), fog.clone())
            });
            frames.push(pair);
        }
    }

...
//!
//! ```text
//! cargo run -p scrap-studio --example shot -- [scene.ron] [out.png] [select name…]
//! ```
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("scrap-studio.png"));
    ...
    let config = std::env::temp_dir().join(format!("scrap-shot-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&config);
    studio.set_config_dir(config.clone());
    if let Ok(preset) = std::env::var("SCRAP_SHOT_THEME") {
        let change = scrap_studio::appearance::Change::Preset(preset);
    ...
    // `SCRAP_SHOT_FLOAT=settings` photographs the Settings panel alone.
    // `SCRAP_SHOT_FLOAT_SIZE=780x560`: the window at the size it opens at.
    let float = std::env::var("SCRAP_SHOT_FLOAT").ok();
    let (fw, fh) = std::env::var("SCRAP_SHOT_FLOAT_SIZE")
        .ok()
        .and_then(|s| {
    ...
    let clicks = std::env::var("SCRAP_SHOT_CLICK").unwrap_or_default();
    // The toolbar as macOS's window has it: the menus in the menu bar.
    if std::env::var_os("SCRAP_SHOT_NATIVE_MENU").is_some() {

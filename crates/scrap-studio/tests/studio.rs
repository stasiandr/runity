...
//!
//! The session's behaviour is tested in `scrap-editor`; what is left here
...
fn scroll_into_view(studio: &mut Studio, node: scrap_ui::NodeId) {
...
fn keys_after_a_line_act_on_it_in_the_scene() {
    ...
    // W E R: the tools.
    key(&mut s, Key::E);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Rotate);
    click(&mut s, "tool Scale");
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Scale);
}
...
fn tool_lit(s: &Studio, name: &str) -> bool {
    ...
    s.ui.style(glyph).text.color == scrap_studio::theme::ACCENT
}
...
fn the_status_bar_shows_the_consoles_newest_line() {
    ...
    use scrap_studio::theme::{ERROR, WARNING};
    let Some((mut s, _dir)) = studio() else {
        return;
    };
...
fn a_long_console_line_opens_on_a_click() {
    ...
    s.session.clear_console();
    s.session.say(
        scrap_editor::console::Level::Error,
...
fn a_terrain_from_the_menu_rises_under_the_brush() {
    ...
    s.session.set_camera(
        scrap::glam::Vec3::new(0.0, 20.0, 20.0),
...
fn project_settings_open_and_save_only_what_reads() {
    let Some((mut s, dir)) = studio() else { return };
    click(&mut s, "tab settings");
    click(&mut s, "settings scrap.ron");
    assert!(s
        .ui
...
fn an_error_does_not_take_the_tab_but_the_status_line_leads_to_it() {
    ...
    click(&mut s, "tab settings");
    s.session
        .say(scrap_editor::console::Level::Error, "something broke");
    s.frame();
    s.refresh();
    s.ui.paint();
    assert!(
...
(elements: [
    ...
    let start = read()
        ...
        .find(|e| e.id == "start")
        .unwrap();
    assert_eq!(start.anchor, scrap::screen::Anchor::BottomRight);
...
fn a_theme_file_recolours_the_running_editor() {
    ...
    let file = dir.join(".scrap/theme.ron");
    std::fs::create_dir_all(file.parent().unwrap()).unwrap();
...
"#,
    ...
    .unwrap();
    let read = || -> scrap::animgraph::Graph {
        scrap::ron::from_str(&std::fs::read_to_string(&file).unwrap()).unwrap()
    };
    ...
    assert!(
        !dir.join(".scrap/animators.ron").exists(),
        "no places kept"
    );
    ...
    assert_eq!(
        ...
        vec![scrap::animgraph::Condition::Below("speed".into(), 0.1)]
    );
...
fn the_animator_lights_up_the_state_the_running_game_is_in() {
    ...
    let state = dir.join(".scrap/state.ron");
    let mut game = std::process::Command::new("sleep");
    game.arg("30").env("SCRAP_STATE_FILE", &state);
    s.session.run_in_console(game).unwrap();
    let boulder = s.session.find("boulder").unwrap();
    let mut report = scrap::save::capture(
        &scrap::hecs::World::new(),
        &scrap::Components::new(),
        &scrap::Scene::default(),
    );
    report.entities.push(scrap::save::Saved {
        id: boulder,
    ...
    for _ in 0..20 {
        std::thread::sleep(std::time::Duration::from_millis(50));
        s.frame();
        if border(&mut s, "state walk") == scrap_studio::theme::WARNING {
            lit = true;
            break;
        }
    }
    assert!(lit, "walk lit while the game is in it");
    assert_ne!(border(&mut s, "state idle"), scrap_studio::theme::WARNING);
    s.session.stop_game();
}
...
fn face_mode_outlines_a_face_and_a_drag_pushes_it() {
    ...
    s.session
        .set_camera(centre + scrap::glam::Vec3::new(2.5, 3.0, 4.0), centre);
    click(&mut s, "faces");
    s.frame();
    let top = scrap::edit::Face::PosY;
...
fn a_panel_floats_in_a_window_of_its_own_and_docks_back() {
    ...
    let target = scrap::OffscreenTarget::new(s.session.gpu(), 400, 500);
    ...
    if let Ok(out) = std::env::var("SCRAP_FLOAT_SHOT") {
    ...
    let surface = scrap_studio::theme::SURFACE;
    assert_eq!(
        at(200, 20),
...
fn an_entity_field_is_picked_from_the_scene_and_a_gone_target_is_named() {
    ...
    let shapes: std::collections::BTreeMap<String, scrap::shape::Shape> = [(
        ...
        scrap::shape::Shape::Struct(vec![
            ("switch".into(), scrap::shape::Shape::Entity),
            ("open".into(), scrap::shape::Shape::Bool),
        ]),
    )]
    .into_iter()
    .collect();
    std::fs::create_dir_all(dir.join(scrap::project::SHAPES).parent().unwrap()).unwrap();
    std::fs::write(
...
fn a_fence_is_shaped_by_dragging_its_points() {
    ...
    let fence = s
        .session
        .add_fence("builtin:cylinder", scrap::glam::Vec3::ZERO, 1.0)
        .unwrap();
    ...
    let spline = |s: &Studio| -> scrap::Spline {
        let text = s
            .session
            .inspect(fence)
            .unwrap()
            ...
            .unwrap()
            .value;
        scrap::ron::from_str(&text).unwrap()
    };
...
fn a_material_instance_is_made_from_the_project_and_is_its_parent_until_changed() {
    ...
    let library = scrap::Library::open(dir.join("library")).unwrap().0;
    assert_eq!(
        library
...
fn a_prefab_field_is_picked_from_the_project_and_a_wrong_one_is_named() {
    ...
    let shapes: std::collections::BTreeMap<String, scrap::shape::Shape> = [(
        "spawner".to_string(),
        scrap::shape::Shape::Struct(vec![
            ("what".into(), scrap::shape::Shape::Asset("prefab".into())),
            ("every".into(), scrap::shape::Shape::Float),
        ]),
    )]
    .into_iter()
    .collect();
...
fn the_animator_shows_what_changed_since_the_commit_and_takes_one_back() {
    ...
    let graph: scrap::animgraph::Graph =
        scrap::ron::from_str(&std::fs::read_to_string(&file).unwrap()).unwrap();
...
fn a_part_of_the_look_switches_on_at_its_default_and_off_to_none() {
    ...
    assert_eq!(
        scrap::ron::from_str::<scrap::ron::Value>(&look(&s, "sky")).ok(),
        scrap::ron::from_str::<scrap::ron::Value>(&s.session.field_blank("sky").unwrap())
            .ok(),
...
fn a_joints_other_body_is_an_entity_picked_dragged_or_clicked_in_the_view() {
    ...
    let at = s.session.transform(ground).unwrap().position + scrap::glam::Vec3::new(4.0, 0.0, 6.0);
...
fn inactive_lines_are_greyed_and_instances_are_in_the_accent() {
    let Some((mut s, _dir)) = studio() else { return };
    use scrap_studio::theme::{ACCENT, TEXT};
    let crate_id = s.session.find("crate").unwrap();
...
fn rect(s: &mut Studio, name: &str) -> scrap_ui::Rect {
    s.ui.paint();
    let node = s
        .ui
        .find(name)
...
fn the_consoles_menu_clears_it_when_play_starts() {
    ...
    s.session.say(scrap_editor::console::Level::Info, "before play");
    s.frame();
    click(&mut s, "play");
    assert!(
    ...
    click(&mut s, "console more");
    click(&mut s, "menu Clear on Play");
    s.session.say(scrap_editor::console::Level::Info, "kept");
    click(&mut s, "play");
...
fn preset_rgb(preset: &str, token: &str) -> [u8; 3] {
    scrap_studio::theme::preset(preset).unwrap().get(token)
}

fn drawn(s: &Studio, c: scrap_ui::Color) -> [u8; 3] {
    let t = s.ui.tint(c);
    [t.r, t.g, t.b]
}
...
fn the_choice_is_the_persons_and_a_new_editor_reads_it() {
    ...
    let file = std::fs::read_to_string(me.join(scrap_studio::appearance::FILE)).unwrap();
    ...
    assert_eq!(
        drawn(&again, scrap_studio::theme::BG),
...
fn a_custom_accent_recolours_the_accent_and_its_ramp() {
    ...
    key(&mut s, Key::Enter);
    assert_eq!(drawn(&s, scrap_studio::theme::ACCENT), [0xe0, 0x7a, 0x5f]);
    assert_ne!(
        drawn(&s, scrap_studio::theme::ACCENT_900),
    ...
    assert_ne!(drawn(&s, scrap_studio::theme::ACCENT), [0xe0, 0x7a, 0x5f]);
    click_page(&mut s, "appearance reset");
    assert_eq!(
        drawn(&s, scrap_studio::theme::ACCENT),
...
fn is_under(s: &Studio, node: scrap_ui::NodeId, root: scrap_ui::NodeId) -> bool {
...
fn command_comma_opens_preferences_in_a_window_of_its_own_on_appearance() {
    ...
    assert_eq!(s.preferences_page(), scrap_studio::preferences::Page::Appearance);
...
fn project_settings_are_the_projects_files_and_no_appearance() {
    ...
    assert!(s.ui.find("settings scrap.ron").is_some());
    ...
    // It opens on scrap.ron rather than on nothing.
...
fn a_rebound_key_works_reads_in_the_menu_and_is_kept_for_the_next_editor() {
    ...
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Rotate);
    key(&mut s, Key::W);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Rotate, "W is no tool now");
    key(&mut s, Key::M);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Move);
    assert_eq!(menu_key(&mut s, "View", "Tool › Move").as_deref(), Some("M"));

    let file = std::fs::read_to_string(me.join(scrap_studio::keymap::FILE)).unwrap();
    ...
    key(&mut again, Key::E);
    key(&mut again, Key::M);
    assert_eq!(again.session.tool(), scrap::gizmo::Tool::Move);
    let _ = std::fs::remove_dir_all(dir2);
}
...
fn a_key_taken_from_another_command_is_its_no_longer_until_reset() {
    ...
    key(&mut s, Key::E);
    key(&mut s, Key::F);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Move, "F moves now");
...
fn the_code_editor_chosen_is_what_a_console_line_opens_with() {
    ...
    let kept = std::fs::read_to_string(me.join(scrap_studio::preferences::FILE)).unwrap();
...
fn saved_layouts_are_renamed_and_deleted_in_preferences() {
    let Some((mut s, dir)) = studio() else { return };
    let me = my_colours(&mut s, &dir);
    let layout = scrap_studio::layouts::Layout::built_in("Tall").unwrap();
    scrap_studio::layouts::save(&me, "Wide", &layout).unwrap();
    shortcut(&mut s, Key::Comma);
    ...
    key(&mut s, Key::Enter);
    assert_eq!(scrap_studio::layouts::saved(&me), ["Painting"]);
    assert!(s.ui.find("layout name Painting").is_some(), "the page shows it");
    click(&mut s, "layout delete Painting");
    assert!(scrap_studio::layouts::saved(&me).is_empty());
    assert!(s.ui.find("layout name Painting").is_none());
}

#[test]
fn t_and_y_are_the_rect_and_transform_tools_as_in_unity() {
    let Some((mut s, _dir)) = studio() else { return };
    click(&mut s, "line crate");
    key(&mut s, Key::T);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Rect);
    key(&mut s, Key::Y);
    assert_eq!(s.session.tool(), scrap::gizmo::Tool::Transform);

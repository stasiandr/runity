...
//!
//! A screen is a RON file in `ui/` (`scrap::screen`): elements anchored
...
use scrap::screen::{Anchor, Element, Kind, Layout};
...
pub struct Screens {
    ...
    renderer: Option<scrap::ui_render::UiRenderer>,
    pub dirty: bool,
...
impl Screens {
    ...
    fn open(&mut self, ui: &mut Ui, session: &mut Session, path: PathBuf) {
        let layout = std::fs::read_to_string(&path)
            ...
            .and_then(|t| scrap::ron::from_str::<Layout>(&t).map_err(|e| e.to_string()));
        match layout {
            Ok(layout) => {
    ...
    fn show_fields(&mut self, ui: &mut Ui) {
        ...
        let values = [
            ...
            ("kind", scrap::ron::to_string(&e.kind).unwrap_or_default()),
        ];
    ...
    fn save(&mut self, session: &mut Session) {
        ...
        let text = patched(&old, layout).unwrap_or_else(|| {
            let pretty = scrap::ron::ser::PrettyConfig::new();
            scrap::ron::ser::to_string_pretty(layout, pretty).unwrap_or_default() + "\n"
        });
        if text != old {
    ...
    fn pick(&self, ui: &Ui, x: f32, y: f32) -> Option<String> {
        ...
        let screen = scrap::glam::Vec2::new(WIDTH, HEIGHT);
        layout
            .elements
            .iter()
            .rev()
        ...
        if let Some(key) = self.boxes.iter().find(|(n, _)| *n == node).map(|(_, k)| *k) {
            ...
            if let Some(e) = self.element_mut() {
                match (key, number) {
                    ...
                    ("kind", _) => match scrap::ron::from_str::<Kind>(&value) {
                        Ok(kind) => e.kind = kind,
    ...
    pub fn draw(&mut self, session: &Session) {
        ...
        if self.target.is_none() {
            ...
            self.renderer = Some(scrap::ui_render::UiRenderer::new(gpu, &target));
        ...
        ui.quad(scrap::ui::Quad::new(
            0.0,
            0.0,
            WIDTH,
            HEIGHT,
            scrap::glam::Vec4::from(BG.linear()),
        ));
        if let Some((_, layout)) = &self.open {
            let mut screen = scrap::screen::Screen::from_layout(layout.clone());
            let mut widgets = scrap::widgets::Widgets::new();
            let input = scrap::input::Input::new();
            screen.draw(
                &mut widgets,
                &mut ui,
                &input,
                scrap::glam::Vec2::new(WIDTH, HEIGHT),
            );
            if let Some(id) = &self.selected {
                if let Some(r) = layout.rect(id, scrap::glam::Vec2::new(WIDTH, HEIGHT)) {
                    let c = scrap::glam::Vec4::from(ACCENT.linear());
                    let t = 2.0;
                    for (x, y, w, h) in [
                        ...
                        ui.quad(scrap::ui::Quad::new(x, y, w, h, c));
                    }
                }
            }
        }
...
fn patched(old: &str, layout: &Layout) -> Option<String> {
    let on_disk: Layout = scrap::ron::from_str(old).ok()?;
    ...
    let check: Layout = scrap::ron::from_str(&text).ok()?;
...
fn element_text(e: &Element) -> String {
    let kind = scrap::ron::to_string(&e.kind).unwrap_or_default();
...
"#;
        let mut layout: Layout = scrap::ron::from_str(old).unwrap();
        ...
        let back: Layout = scrap::ron::from_str(&new).unwrap();

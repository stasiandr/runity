...
//! the shape the game wrote down. A field that names an asset — a model, a
//! material, a sound — is Unity's object field ([`object`]): picked or
//! dragged onto, never typed. Empty fields are offered by Add Component.
//!
...
pub const PREVIEW: scrap_ui::ImageId = scrap_ui::ImageId(1);
...
pub const SV_SQUARE: scrap_ui::ImageId = scrap_ui::ImageId(10);
pub const HUE_STRIP: scrap_ui::ImageId = scrap_ui::ImageId(11);
const PICTURE: u32 = 128;

/// X, Y and Z as the gizmo colours them.
const AXES: [scrap_ui::Color; 3] = [
    scrap_ui::Color::hex(0xe58a96),
    scrap_ui::Color::hex(0x9fd49a),
    scrap_ui::Color::hex(0x8fb4e8),
];
...
/// The kind of asset a field of the line names, when it is one: an
/// object field, not a box to type a name into.
fn object_kind(f: &Field) -> Option<&'static str> {
...
/// A field's name as a label: `ambient_occlusion` as "Ambient occlusion",
/// `components.door` as "Door".
...
enum Part {
    ...
    Revert(String),
    /// An object field, or its ◎: opens the picker of its kind's assets.
    Object(ObjectRef),
...
enum Scrub {
    ...
    /// A number of a form; `true` for a count.
    Form(Place, bool),
}
...
fn to_srgb(c: [f32; 3]) -> [u8; 3] {
    c.map(|v| (scrap::material::linear_to_srgb(v.clamp(0.0, 1.0)) * 255.0).round() as u8)
}
...
pub struct Inspector {
    ...
    images: Vec<(scrap_ui::ImageId, u32, Vec<u8>)>,
    hue_drawn: bool,
    /// Each component field's box, to show a scrubbed number as it goes.
    sub_nodes: HashMap<(String, String), NodeId>,
    /// Debug mode: every field as the RON its file holds. `None` until
    /// read from this person's preferences.
    debug: bool,
    debug_read: bool,
    /// Debug mode's box, when it shows one.
    debug_box: Option<debug::DebugBox>,
    /// Every object field laid out, for a drag from the Project to find.
    objects: Vec<(NodeId, ObjectRef)>,
    /// The Project's pictures drawn so far, by what each is asked for by.
    pictures: HashMap<String, scrap_ui::ImageId>,
    ...
    /// inside it writes in around its own number.
    stand_ins: HashMap<Place, String>,
    /// Each form number's box, to show a scrubbed number as it goes.
    form_nodes: HashMap<Place, NodeId>,
    /// The label of the form line being laid out: a number's handle.
    last_label: Option<NodeId>,
    /// What the forms were laid out from: a value changed under them (an
    /// undo, an edit elsewhere) lays them out again.
...
impl Inspector {
    ...
    /// The Project's pictures drawn so far: what an object field shows its
    /// asset as.
    pub fn set_pictures(&mut self, pictures: HashMap<String, scrap_ui::ImageId>) {
        if pictures.len() != self.pictures.len() {
    ...
    pub fn take_images(&mut self) -> Vec<(scrap_ui::ImageId, u32, Vec<u8>)> {
    ...
    pub fn update(&mut self, ui: &mut Ui, session: &Session) {
        ...
        // is being typed into, which a new layout would take away.
        let form_values: Vec<String> = if ids.is_empty() {
            session.environment().into_iter().map(|(_, v)| v).collect()
        } else {
            fields
                .iter()
                .filter(|f| self.is_form(f) || object_kind(f).is_some())
                .map(|f| f.value.clone())
                .collect()
        };
        let typing = ui
            .focused()
            .is_some_and(|n| matches!(self.parts.get(&n), Some(Part::Form(..))));
        if ids != self.showing {
            self.revealed.clear();
        ...
        let changed = !self.built
            ...
            || resets != self.resets
            || (form_values != self.form_values && !typing);
        if changed && !dragging {
    ...
    fn build(&mut self, ui: &mut Ui, session: &Session, ids: &[EntityId], fields: &[Field]) {
        ...
        if let Some(inactive) = find("inactive") {
            ...
            let check = ui.add(
                ...
                Style::row()
                    ...
                    .background(if on { ACCENT } else { scrap_ui::Color::TRANSPARENT })
                    .hover_border(ACCENT)
                    .clickable(),
            );
        ...
        for (name, group) in [
            ...
            for f in group {
                ...
                if f.name == "material" && f.value != MIXED && !self.is_form(f) {
    ...
    fn line(&mut self, ui: &mut Ui, session: &Session, f: &Field) {
        ...
        self.label_text(
            ...
            label,
            &title(&f.name),
            if f.overridden { ACCENT_300 } else { LABEL },
        );
        match TRANSFORM
    ...
    fn component_form(&mut self, ui: &mut Ui, session: &Session, f: &Field) -> bool {
        ...
        for (key, kind) in &shape {
            ...
            let name = ui.add(
                line,
                Style::row()
                    .width(form::LABEL_WIDTH - 18.0)
                    .fixed()
                    .min_height(22.0)
                    .center_items()
                    .radius(RADIUS_SM),
            );
            self.label_text(ui, name, &title(key), LABEL);
            ...
            if sub == SubKind::Raw {
                // A struct, a list, an option in it: a form of its own.
                self.component_part(ui, session, line, f, key, kind);
                continue;
            }
            let node = match &sub {
                SubKind::Bool(on) => {
    ...
    fn set_sub(&mut self, session: &mut Session, component: &str, key: &str, value: &str) {
        let Some(scrap::shape::Shape::Struct(shape)) = self.shapes.get(component).cloned() else {
            return;
        };
        let values = self
            .component_values
    ...
    fn shader_properties(&mut self, ui: &mut Ui, session: &Session, material: &str, fields: &[(String, String, bool)]) {
        ...
        let properties = scrap::render::material_properties(&project.root().join(scrap::project::SHADERS), &shader);
    ...
    pub fn event(
        ...
        match (part, event) {
            ...
            (Part::Object(target), Event::Click { .. }) => {
                // The field's rectangle, whether its ◎ or itself was hit.
                let field = self
                    .objects
                    .iter()
                    .find(|(_, t)| *t == target)
            ...
            (Part::Scrub(scrub), Event::Press { .. }) => {
                let start = match &scrub {
                    ...
                    Scrub::Form(place, _) => self.number_at(session, place),
                };
    ...
    fn color_of(&self, session: &Session, target: &ColorTarget) -> Option<[u8; 3]> {
        if let ColorTarget::Form(place) = target {
            return self.colour_at(session, place);
        }
    ...
    fn apply_color(&mut self, session: &mut Session, target: &ColorTarget) {
        ...
        match target {
            ColorTarget::Material => {
                let material = scrap::Material::from_srgb(r, g, b);
                for id in self.showing.clone() {
            ...
            ColorTarget::Form(place) => {
                let rgb = [r, g, b].map(|v| v as f32 / 255.0);
                self.set_colour(session, place, rgb);
            }
        }
    }
    ...
    fn open_add(&mut self, ui: &mut Ui, session: &Session, at: scrap_ui::Rect) {
        ...
        for (i, (addition, label)) in offered.into_iter().enumerate() {
            let row = ui.add(
                ...
                Style::row()
                    ...
                    .center_items()
                    .radius(RADIUS_SM)
                    .background(if i == 0 && !query.is_empty() { HOVER } else { scrap_ui::Color::TRANSPARENT })
                    .hover(HOVER)
                    .clickable(),
            );
    ...
    fn open_picker(&mut self, ui: &mut Ui, target: ColorTarget, rgb: [u8; 3], at: scrap_ui::Rect) {
        self.hsv = hsv_of(rgb);
        ...
        let square_mark = ui.add(
            ...
            Style::row()
                ...
                .size(10.0, 10.0)
                .radius(5.0)
                .border(2.0, scrap_ui::Color::hex(0xffffff)),
        );
    ...
    fn show_picker(&mut self, ui: &mut Ui) {
        ...
        ui.restyle(*preview, |st| {
            st.background(scrap_ui::Color::rgba(rgb[0], rgb[1], rgb[2], 255))
        });

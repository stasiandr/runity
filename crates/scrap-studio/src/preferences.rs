...
//! with ⌘, (Ctrl+, elsewhere), from «scrap › Settings…» on a Mac or
...
//! `.scrap/editor.ron` (`scrap_editor::prefs`): they mean nothing in
//! another project.
//!
...
pub struct Personal {
    ...
    /// Blender's executable. Empty: found as `scrap_import::blend::blender`
    /// finds it (`SCRAP_BLENDER`, where it installs, the `PATH`).
    pub blender: String,
...
impl Personal {
    ...
    pub fn load(dir: Option<&Path>) -> (Self, Option<String>) {
        ...
        match std::fs::read_to_string(&file) {
            Ok(text) => match scrap::ron::from_str(&text) {
                Ok(this) => (this, None),
...
impl Preferences {
    ...
    pub fn show_personal(&mut self, ui: &mut Ui, personal: &Personal) {
        ...
        let found = if personal.blender.trim().is_empty() {
            match scrap_import::blend::blender() {
...
mod tests {
    ...
    fn preferences_read_back_and_default_what_they_do_not_say() {
        let dir = std::env::temp_dir().join(format!("scrap-prefs-{}", std::process::id()));

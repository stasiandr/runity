...
//! (`scrap_editor::actions`: Save, Undo, Delete…), the menus' (Copy,
...
pub fn commands() -> Vec<Command> {
    ...
    for a in scrap_editor::actions::registry() {
...
impl Keymap {
    ...
    pub fn load(dir: Option<&Path>) -> (Self, Vec<String>) {
        ...
        let map: BTreeMap<String, Vec<String>> = match scrap::ron::from_str(&text) {
            Ok(map) => map,
            Err(e) => {
                errors.push(format!("{}: {e}", file.display()));
                return (this, errors);
...
mod tests {
    ...
    fn only_what_differs_is_written_and_it_reads_back() {
        let dir = std::env::temp_dir().join(format!("scrap-keymap-{}", std::process::id()));
        let mut map = Keymap::default();

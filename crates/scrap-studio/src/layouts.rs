...
//!
//! One format for three things: the project's `.scrap/studio.ron` (the
...
/// Support/scrap` on macOS, `%APPDATA%\scrap` on Windows,
/// `$XDG_CONFIG_HOME/scrap` or `~/.config/scrap` elsewhere — or
/// `SCRAP_CONFIG_DIR`. Not made here: nothing needs it until a layout is
/// saved.
...
pub struct Layout {
    ...
    /// How the Project panel shows its assets (`Bottom::modes`): kept with
    /// the layout in the project's file too, not in a preset.
...
impl Layout {
    ...
    pub fn write(&self) -> String {
        ...
        if let Some(modes) = &self.project {
            out.push_str(&format!("project: {modes:?}, "));
        }

...
fn scene_to_open(bundled: bool) -> Option<PathBuf> {
    ...
    let reference = PathBuf::from(scrap_studio::REFERENCE_SCENE);
    if !bundled && reference.is_file() {
        return Some(reference);
    }
    // Unless Preferences › General says to start from a pick every time.
    let config = scrap_studio::appearance::config_dir();
    let (personal, _) = scrap_studio::preferences::Personal::load(config.as_deref());
...
/// Started from `Scrap.app`: Finder gave it `/` as the directory and a
...
fn memory_file() -> Option<PathBuf> {
    ...
    Some(home.join("Library/Application Support/scrap/last-scene"))
}

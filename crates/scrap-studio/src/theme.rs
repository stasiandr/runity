...
//!
//! Icons are Lucide, shipped by `scrap-ui`. Nocturne names Phosphor; the
//! two share a 24-unit grid and stroke, and swapping sets is swapping files.

use std::collections::{BTreeMap, HashMap};

use scrap_ui::{Color, NodeId, Style, Ui};
...
pub fn parse(text: &str) -> Result<(Option<String>, Tokens), String> {
    let map: BTreeMap<String, String> = scrap::ron::from_str(text).map_err(|e| e.to_string())?;
    let mut named = None;

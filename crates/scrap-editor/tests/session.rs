...
use scrap::gizmo::{Grip, Handle, Tool};
use scrap::glam::Vec3;
...
fn loose_scene_file(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("scrap-editor-loose-{name}"));
...
fn a_transform_round_trips_and_reaches_the_file() {
    ...
    let reopened = scrap::Scene::load(&path).unwrap();
    let lid = reopened
        .flatten()
        .into_iter()
        .find(|(e, _)| e.name == "lid")
...
fn an_entity_can_be_pointed_at_the_palette_and_reports_what_it_became() {
    ...
    assert_eq!(
        session.material(crate_id),
        Some(scrap::material::builtin::STONE),
...
fn the_palette_offers_the_builtins_and_the_librarys_own() {
    ...
    assert_eq!(builtins.len(), scrap::material::builtin::NAMES.len());
    ...
    let library = std::env::temp_dir().join("scrap-editor-palette-library");
...
fn write_material(directory: &Path, name: &str, material: Material) {
    let bytes = scrap::asset::to_bytes(
        &scrap::MaterialAsset {
            id: scrap::AssetId::from_source(name, 0),
            name: name.into(),
            material,
        },
...
fn opening_a_scene_looks_where_the_scene_says_and_keeping_a_view_is_a_decision() {
    ...
    let default = scrap::View::default();
    ...
    session.save_scene(None).unwrap();
    assert_eq!(
        scrap::Scene::load(&path).unwrap().view(),
        default,
...
fn pause_holds_the_crate_in_the_air_and_step_moves_it_one_step() {
    use scrap::input::{Input, InputEvent as E, Key};
    ...
    let shown = session
        ...
        .unwrap()
        .value;
    let y: Vec3 = scrap::ron::from_str(&shown).unwrap();
    assert!(
...
fn a_model_dropped_on_the_editor_becomes_something_a_scene_can_use() {
    ...
    assert!(scrap::Library::open(&library)
        .unwrap()
        .0
        .mesh_by_name("wedge")
        .is_some());
...
fn a_scene_in_no_project_still_edits_and_says_what_it_cannot_do() {
    ...
    assert!(err.to_string().contains("scrap.ron"), "{err}");
    assert_eq!(
...
fn a_level_answers_whether_the_exit_can_be_reached() {
    ...
    let settings = scrap::navigation::NavSettings::default();
    let path = session
...
fn renaming_a_material_keeps_the_open_scene_and_its_undo_pointing_at_it() {
    ...
    let used = session.asset_usages("materials/clay.scrmat").unwrap();
...
fn an_asset_the_open_scene_uses_cannot_be_deleted_even_unsaved() {
    ...
    assert!(listed
        .iter()
        .any(|e| e.file == "materials/clay.scrmat" && e.uses == 0));
    ...
    session
        .duplicate_asset("materials/clay.scrmat", "materials/brick.scrmat")
        .unwrap();
    session.set_material_name(lid, "brick").unwrap();
    session.delete_asset("materials/clay.scrmat").unwrap();
    assert!(!root.join("materials/clay.scrmat").exists());
    assert_eq!(
        session.material(lid),
        Some(scrap::Material::from_srgb(0xb4, 0x64, 0x3c)),
        "drawn with the copy"
    );
}
...
fn problems_are_known_as_soon_as_an_edit_makes_them() {
    ...
    session
        .add_entity(
            ...
            .with(scrap::scene::ModelRef("builtin:cub".into())),
        )
        .unwrap();
    session
        .add_entity(
...
fn an_unpacked_instance_is_plain_entities_and_stops_following_the_prefab() {
    ...
    assert_eq!(
        part.material_ref(),
        scrap::scene::MaterialRef::Named("moss".into()),
...
fn view_frame(
    session: &mut Session,
    input: &mut scrap::input::Input,
    at: (f32, f32),
    events: &[scrap::input::InputEvent],
) -> Vec<&'static str> {
    input.begin_frame();
    input.handle(&scrap::input::InputEvent::MouseMoved { x: at.0, y: at.1 });
    for e in events {
        input.handle(e);
    }
...
fn the_scene_view_does_what_a_mouse_and_keyboard_ask() {
    use scrap::input::{Input, InputEvent as E, Key, MouseButton as M};
    ...
    assert_eq!(session.tool(), scrap::gizmo::Tool::Rotate);
    view_frame(
        &mut session,
        &mut input,
        centre,
    ...
    assert_eq!(session.tool(), scrap::gizmo::Tool::Move);
...
fn an_axis_view_is_a_plan_that_pans_zooms_and_picks() {
    ...
    assert_eq!(session.camera().up, scrap::glam::Vec3::Y);
...
fn the_hand_pans_and_neither_picks_nor_moves() {
    ...
    session.select(Some(crate_id)).unwrap();
    session.focus_selected();
    session.set_tool(scrap::gizmo::Tool::Rotate);
    let (w, h) = session.size();
    let centre = (w as f32 / 2.0, h as f32 / 2.0);
...
fn a_right_drag_looks_and_wasd_flies_without_touching_the_tools() {
    ...
    session.look_from(scrap_editor::Side::Top);
...
fn ctrl_shift_puts_the_selection_on_whatever_the_cursor_is_over() {
    ...
    let over = |session: &Session, p: scrap::glam::Vec3| {
    ...
    let (x, y) = over(&session, scrap::glam::Vec3::new(4.3, 1.0, 0.2));
    ...
    let (x, y) = over(&session, scrap::glam::Vec3::new(4.0, 1.0, 0.0));
    let did = view_frame(
        &mut session,
        &mut input,
        (x as f32, y as f32),
    ...
    session
        .set_transform(
            crate_id,
            scrap::Transform {
                position: scrap::glam::Vec3::new(4.0, 5.0, 0.0),
                ..Default::default()
            },
        )
        .unwrap();
...
fn a_handle_moves_turns_and_stretches_everything_selected() {
    ...
    session.set_tool(scrap::gizmo::Tool::Scale);
...
fn local_handles_slide_a_turned_wall_along_its_own_length() {
    ...
    assert_eq!(session.space(), scrap_editor::Space::Local);
...
fn in_center_mode_the_selection_turns_about_its_middle() {
    ...
    session.select(Some(crate_id)).unwrap();
    session.add_to_selection(barrel).unwrap();
    session.set_pivot(scrap_editor::Pivot::Center);
...
fn the_inspector_edits_several_things_at_once_in_one_step() {
    ...
    let value = |fields: &[scrap_editor::panels::Field], name: &str| {
        fields
            .iter()
            .find(|f| f.name == name)
            .unwrap()
    ...
    assert_eq!(value(&fields, "name"), scrap_editor::panels::MIXED);
    assert_eq!(value(&fields, "position"), scrap_editor::panels::MIXED);
...
fn a_hierarchy_drag_reorders_and_reparents_without_moving_anything_in_the_world() {
    ...
    session
        .set_transform(
            crate_id,
            scrap::Transform {
                position: scrap::glam::Vec3::new(2.0, 0.5, 0.0),
                rotation_deg: scrap::glam::Vec3::new(0.0, 90.0, 0.0),
                ..Default::default()
            },
        )
        .unwrap();
...
fn view_frame_result(
    ...
    input: &mut scrap::input::Input,
    events: &[scrap::input::InputEvent],
) -> Result<Vec<&'static str>, scrap_editor::EditError> {
    input.begin_frame();
    for e in events {
        input.handle(e);
    }
...
fn the_view_shows_where_a_walker_can_go_and_keeps_up_with_edits() {
    ...
    let wall = id(&session, "wall");
    session.set_show_navigation(Some(scrap::navigation::NavSettings::default()));
...
fn the_view_is_where_this_person_left_it_and_the_scene_file_does_not_know() {
    ...
    session.set_snap(scrap_editor::Snap {
        meters: 0.5,
        degrees: 0.0,
        scale: 0.0,
    });
    ...
    drop(session);
    let project = scrap::Project::find(&path).unwrap();
    assert_eq!(scrap_editor::Session::last_scene(&project), Some(cave));
    let ignore = std::fs::read_to_string(project.root().join(".gitignore")).unwrap();
    assert!(ignore.contains("/.scrap/"), "one person's, not the team's");
}
...
fn play_in_the_game_saves_the_scene_and_names_it_to_the_game() {
    ...
    let live = command
        .get_envs()
        .find(|(k, _)| *k == "SCRAP_SCENE_FILE")
        .and_then(|(_, v)| v)
        .map(PathBuf::from)
        .unwrap();
    assert!(live.starts_with(root_of(&path).join(".scrap")), "{live:?}");
    let given = |live: &Path| -> scrap::Scene {
        scrap::ron::from_str(&std::fs::read_to_string(live).unwrap()).unwrap()
    };
    assert!(given(&live).find("box").is_some());
    // And the game can play it from there: the project is found above it.
    let (_, problems) = scrap::LiveScene::open(&live).unwrap();
    assert!(problems.is_empty(), "{problems:?}");
    let mut game = std::process::Command::new("sleep");
    game.arg("30").env("SCRAP_SCENE_FILE", &live);
    ...
    let state = command
        .get_envs()
        .find(|(k, _)| *k == "SCRAP_STATE_FILE")
        .and_then(|(_, v)| v)
        .map(PathBuf::from)
        .unwrap();
    let mut game = std::process::Command::new("sleep");
    game.arg("30")
        .env("SCRAP_SCENE_FILE", &live)
        .env("SCRAP_STATE_FILE", &state);
    session.run_in_console(game).unwrap();
    let box_id = id(&session, "box");
    assert!(session.game_state().is_none(), "nothing said yet");
    scrap::save::SaveGame {
        entities: vec![scrap::save::Saved {
            id: box_id,
            transform: Transform {
...
fn a_selected_light_shows_how_far_it_reaches() {
    ...
    session.set_camera(
        scrap::glam::Vec3::new(0.0, 12.0, 0.1),
        scrap::glam::Vec3::ZERO,
    );
...
fn poly(points: &[(f32, f32)], height: f32) -> scrap_import::poly::PolySource {
    scrap_import::poly::PolySource {
...
fn the_face_under_the_pointer_is_the_one_facing_it() {
    ...
    assert_eq!(
        ...
        Some((block, scrap::edit::Face::PosZ))
    );
    // From above: the top.
    session.set_camera(Vec3::new(0.0, 8.0, 0.01), Vec3::new(0.0, 0.5, 0.0));
    assert_eq!(
        session.face_under(w / 2, h / 2),
        Some((block, scrap::edit::Face::PosY))
    );
    ...
    let (corners, out) = session
        .face_on_screen(block, scrap::edit::Face::PosY)
        .unwrap();
    let (x0, x1) = corners
        .iter()
...
fn a_material_link_gets_its_id_and_a_builtin_stays_a_name() {
    ...
    std::fs::rename(
        project.materials().join("clay.scrmat"),
        project.materials().join("terracotta.scrmat"),
    )
    .unwrap();
    scrap_import::sync(&project);
    session.open_scene(&path).unwrap();
...
fn an_open_blender_sends_its_saves_and_its_drags_over_the_link() {
    let Some(blender) = scrap_import::blend::blender() else {
...
#[test]
fn the_transform_tool_is_y_and_each_of_its_parts_does_its_own_drag() {
    use scrap::input::{Input, InputEvent as E, Key};
    ...
    let (w, h) = session.size();
    let centre = (w as f32 / 2.0, h as f32 / 2.0);
    view_frame(&mut session, &mut input, centre, &[E::KeyDown(Key::Y)]);
    assert_eq!(session.tool(), Tool::Transform);
    view_frame(
        &mut session,
        &mut input,
        centre,
        &[E::KeyUp(Key::Y), E::KeyDown(Key::T)],
    );
...
fn a_tuning_file_of_records_is_a_table_whose_cells_change_only_themselves() {
    ...
    session
        .set_cell("configs/enemies.ron", "orc", "hp", "35")
        .unwrap();
    assert_eq!(
    ...
    components
        ...
        .unwrap();
    let table = session.table("configs/enemies").unwrap();
    assert_eq!(table.columns[0].shape, "whole number");
    let e = session
        .set_cell("configs/enemies.ron", "orc", "hpp", "1")
        .unwrap_err()
        .to_string();
    assert!(e.contains("did you mean `hp`?"), "{e}");
    let e = session
        .set_cell("configs/enemies.ron", "orc", "hp", "\"lots\"")
        .unwrap_err()
        .to_string();

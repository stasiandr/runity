...
//! grid shows.
//!
//! Unity keeps this in `Library/`; here it is `.scrap/editor.ron` in the
...
pub const FILE: &str = ".scrap/editor.ron";
...
impl Session {
    ...
    /// or a perfect one with `""`. Refuses what `SCRAP_LINK` would not
    /// read.
    pub fn set_link(&mut self, link: &str) -> crate::EditResult<()> {
        scrap::net::Conditions::parse(link).map_err(crate::EditError::Scene)?;
        let Some(path) = self.prefs_path() else {
    ...
    pub fn last_scene(project: &scrap::Project) -> Option<PathBuf> {
        let text = std::fs::read_to_string(project.root().join(FILE)).ok()?;
        let prefs: Prefs = scrap::ron::from_str(&text).ok()?;
...
fn write_prefs(path: &std::path::Path, prefs: &Prefs) {
    if let Ok(text) = scrap::ron::ser::to_string_pretty(prefs, Default::default()) {

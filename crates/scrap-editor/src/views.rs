...
impl Session {
    ...
    pub(crate) fn handle_orientation(&self) -> scrap::glam::Quat {
    ...
    fn translate_selection_world(&mut self, offset: Vec3) -> crate::EditResult<()> {
        ...
        let moves: Vec<(scrap::EntityId, Vec3)> = self
            .selection_roots()
            .into_iter()
    ...
    pub fn align_with_view(&mut self) -> crate::EditResult<bool> {
        ...
        let turn =
            scrap::glam::Quat::from_mat3(&scrap::glam::Mat3::from_cols(right, up, forward));
        let world = scrap::glam::Mat4::from_rotation_translation(turn, self.camera.position);
    ...
    pub fn look_through(&mut self, camera: scrap::Camera) {
        self.camera = camera;
    }

...
use scrap::prelude::*;
use scrap::scene::{Body, BodyProps, Collider, EntityDesc, Joint, Lens, MaterialRef};
use scrap::EntityId;
...
fn take_field(
    from: &mut scrap::scene::Override,
    field: &str,
) -> EditResult<scrap::scene::Override> {
    let mut one = scrap::scene::Override::default();
    match field {
...
fn part_kind(field: &str) -> Option<scrap::parts::PartKind> {
...
fn pretty<T: serde::Serialize>(value: &T, depth: usize) -> EditResult<String> {
    let config = scrap::ron::ser::PrettyConfig::new().depth_limit(depth);
    scrap::ron::ser::to_string_pretty(value, config).map_err(|e| EditError::Scene(e.to_string()))
}
...
fn parse<T: serde::de::DeserializeOwned>(field: &str, text: &str) -> EditResult<T> {
    scrap::ron::from_str(text).map_err(|e| EditError::Scene(format!("{field}: {e}")))
}
...
impl Session {
    ...
    pub fn changed_since(&self, committed: Option<&str>) -> std::collections::HashSet<EntityId> {
        let mut then: Option<scrap::Scene> =
            committed.and_then(|text| scrap::ron::from_str(text).ok());
        ...
        if let Some(then) = &mut then {
            scrap::refs::settle(&mut then.entities, self.library.as_ref(), &self.prefabs);
        }
    ...
    fn live_transform(&self, id: EntityId) -> Option<scrap::scene::Transform> {
        if !self.is_playing() {
            return None;
        }
        self.world
            .query::<(&scrap::SceneId, &scrap::scene::Transform)>()
            .iter()
    ...
    pub fn component_shapes(&self) -> std::collections::BTreeMap<String, scrap::shape::Shape> {
        self.project()
            .map(|p| p.root().join(scrap::project::SHAPES))
    ...
    /// an enum's variants ([`scrap::shape`]). For a game component, the
    ...
    pub fn field_shape(&self, field: &str) -> Option<scrap::shape::Shape> {
    ...
    pub fn field_variants(&self, field: &str) -> Vec<(String, scrap::shape::Shape)> {
    ...
    pub fn entity_ron(&self, id: EntityId) -> EditResult<String> {
        ...
        Ok(scrap::ron_edit::set_field(&text, "children", None).unwrap_or(text))
    }
    ...
    pub fn set_entity_ron(&mut self, id: EntityId, text: &str) -> EditResult<()> {
        ...
        let mut next: EntityDesc =
            scrap::ron::from_str(text).map_err(|e| EditError::Scene(e.to_string()))?;
    ...
    pub fn scene_settings_ron(&self) -> EditResult<String> {
        let settings = scrap::scene::Scene {
            parts: self.history.scene().parts.clone(),
            entities: Vec::new(),
        };
        let text = pretty(&settings, 1)?;
        Ok(scrap::ron_edit::set_field(&text, "entities", None).unwrap_or(text))
    }
    ...
    pub fn add_component(&mut self, id: EntityId, name: &str) -> EditResult<String> {
        ...
        let shape = shapes.get(name).ok_or_else(|| {
            EditError::Scene(format!(
                ...
                scrap::spelling::closest(name, shapes.keys().map(String::as_str))
    ...
    fn part_override(&self, id: EntityId) -> Option<scrap::scene::Override> {
    ...
    pub fn set_field(&mut self, id: EntityId, field: &str, text: &str) -> EditResult<()> {
        ...
        match field {
            ...
            "model" => next.set_part(&scrap::scene::ModelRef(text.into())),
            "prefab" => next.prefab = text.into(),
            "layer" => next.set_part(&scrap::scene::LayerName(text.to_string())),
            "inactive" => next.inactive = parse::<bool>(field, text)?,
            "animator" => next.set_part(&scrap::scene::AnimatorRef(text.trim().to_string())),
            "bends_grass" => next.set_part(&scrap::scene::BendsGrass(
                parse::<f32>(field, text)?.max(0.0),
            )),
            "bone" => next.set_part(&scrap::scene::BoneName(text.trim().to_string())),
            ...
            "light" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Light>(field, text)?)
                })
                .as_ref(),
            ),
            "spline" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::Spline>(field, text)?)
                })
                .as_ref(),
            ),
            "along" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::Along>(field, text)?)
                })
                .as_ref(),
            ),
            "route" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Route>(field, text)?)
                })
                .as_ref(),
            ),
            "soft_body" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::SoftBody>(field, text)?)
                })
                .as_ref(),
            ),
            "jiggle" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Jiggle>(field, text)?)
                })
                .as_ref(),
            ),
            "fluid" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Fluid>(field, text)?)
                })
                .as_ref(),
            ),
            "fracture" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Fracture>(field, text)?)
                })
                .as_ref(),
            ),
            "dents" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Dents>(field, text)?)
                })
                .as_ref(),
            ),
            "mpm" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Mpm>(field, text)?)
                })
                .as_ref(),
            ),
            "shallow_water" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::ShallowWater>(field, text)?)
                })
                .as_ref(),
            ),
            "ripples" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Ripples>(field, text)?)
                })
                .as_ref(),
            ),
            "ocean" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Ocean>(field, text)?)
                })
                .as_ref(),
            ),
            "floats" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Floats>(field, text)?)
                })
                .as_ref(),
            ),
            "smoke" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Smoke>(field, text)?)
                })
                .as_ref(),
            ),
            "grains" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Grains>(field, text)?)
                })
                .as_ref(),
            ),
            "distance_field" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::DistanceField>(field, text)?)
                })
                .as_ref(),
            ),
            "snow_cover" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::SnowCover>(field, text)?)
                })
                .as_ref(),
            ),
            "ragdoll" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Ragdoll>(field, text)?)
                })
                .as_ref(),
            ),
            "crawler" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Crawler>(field, text)?)
                })
                .as_ref(),
            ),
            "rope" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Rope>(field, text)?)
                })
                .as_ref(),
            ),
            "cloth" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Cloth>(field, text)?)
                })
                .as_ref(),
            ),
            "hair" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Hair>(field, text)?)
                })
                .as_ref(),
            ),
            "particles" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Emitter>(field, text)?)
                })
                .as_ref(),
            ),
            "decal" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Decal>(field, text)?)
                })
                .as_ref(),
            ),
            "sound" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::SoundSource>(field, text)?)
                })
                .as_ref(),
            ),
            "footprints" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::footprints::Footprints>(field, text)?)
                })
                .as_ref(),
            ),
            "terrain" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::terrain::Terrain>(field, text)?)
                })
                .as_ref(),
            ),
            "render_texture" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::RenderTexture>(field, text)?)
                })
                .as_ref(),
            ),
            "post_volume" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::PostVolume>(field, text)?)
                })
                .as_ref(),
            ),
            "reflection_probe" => next.set_part_opt(
                (if text.trim() == "None" {
                    ...
                    Some(parse::<scrap::scene::Probe>(field, text)?)
                })
                .as_ref(),
            ),
            other => {

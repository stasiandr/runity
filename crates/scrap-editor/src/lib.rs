...
use scrap::gizmo::{self, Drag, GizmoMeshes, GizmoStyle, Grip, Handle, Motion, Tool};
use scrap::glam::{Mat4, Vec3};
use scrap::render::{Camera, FogSettings, Frame, Lighting, MeshHandle};
...
pub struct Session {
    ...
    shaders: Option<scrap::render::MaterialShaders>,
    ...
    /// drawing meanwhile.
    syncing: Option<(
        std::thread::JoinHandle<Vec<scrap_import::Reimported>>,
        std::time::Instant,
    )>,
    ...
    drag_from: Option<scrap::Transform>,
    ...
    drag_others: Vec<(EntityId, scrap::Transform)>,
    /// Which way the handles pointed when the drag began.
    drag_orientation: scrap::glam::Quat,
    ...
    marquee: Option<(scrap::glam::Vec2, scrap::glam::Vec2)>,
    ...
    /// next drawn).
    nav_shown: Option<(
        scrap::navigation::NavSettings,
        Option<scrap::navigation::NavGrid>,
    )>,
...
impl Session {
    ...
    pub fn offscreen(width: u32, height: u32) -> EditResult<Self> {
        ...
        Ok(Self {
            ...
            prefab_dir: None,
            instanced: scrap::Instanced::default(),
            uploaded: Vec::new(),
            ...
            drag_from: None,
            drag_others: Vec::new(),
            drag_orientation: scrap::glam::Quat::IDENTITY,
            space: Space::Global,
            pivot: Pivot::Pivot,
            gizmo_arm: None,
            gizmo_meshes: None,
    ...
    pub fn open_scene(&mut self, path: impl AsRef<Path>) -> EditResult<Vec<String>> {
        ...
        // nobody can predict.
        self.camera = scrap::scene_camera(&scene.view());
        ...
        let project = scrap::Project::find(&path).ok();
        let (prefabs, problems) = project
            .as_ref()
            .map(scrap::Prefabs::of)
            .unwrap_or_default();
        ...
        let stamps = scrap::live::stamps(&path, self.project.as_ref());
    ...
    pub fn save_scene(&mut self, path: Option<&Path>) -> EditResult<()> {
        ...
        if self.scene_path.as_ref() == Some(&target) {
            let stamps = scrap::live::stamps(&target, self.project.as_ref());
    ...
    /// opposite face stays put (see [`scrap::edit::push_face`]). One undo
    /// step; on a prefab's part, an override like any other edit of it.
    pub fn push_face(
        &mut self,
        id: EntityId,
        face: scrap::edit::Face,
        metres: f32,
    ) -> EditResult<()> {
        ...
        let pushed =
            scrap::edit::push_face(&line.transform, min, max, face, metres).ok_or_else(|| {
                EditError::Scene(format!(
    ...
    pub fn snap_selection(&mut self) -> EditResult<usize> {
        ...
        let snapped: Vec<(EntityId, scrap::scene::Transform)> = roots
            .iter()
            .filter_map(|id| {
    ...
    pub(crate) fn solid_without(&self, roots: &[EntityId]) -> scrap::PhysicsWorld {
        ...
        fn solidify(entities: &mut [EntityDesc], moving: &std::collections::HashSet<EntityId>) {
            for e in entities {
                if moving.contains(&e.id) || e.body() == scrap::Body::Trigger {
                    e.set_part(&scrap::Body::None);
                } else {
                    e.set_part(&scrap::Body::Static);
                    if e.collider() == scrap::scene::Collider::None {
                        e.set_part(&scrap::scene::Collider::Model);
                    }
                }
        ...
        scrap::spawn_scene(&ground, &mut world, |_| Some(MeshHandle::TEST));
        scrap::physics::attach_scene_collision_meshes(&mut world, &ground, self.library.as_ref());
    ...
    /// a line of the terrain's `.scrterrain` and rebuilt at once. `by` raises
    ...
    pub fn sculpt(
        ...
        scrap_import::walk(&project.assets(), &mut |path| {
            ...
            if named && path.extension().is_some_and(|e| e == "scrterrain") {
                source = Some(path.to_path_buf());
            }
        });
        let source = source.ok_or_else(|| {
            EditError::Scene(format!(
                "`{}` does not draw a terrain: no {}.scrterrain in assets/",
                desc.name,
                desc.model()
            ))
        })?;
        ...
        let edit = if flatten {
            scrap_import::terrain::Edit::Flatten {
                at: (local.x, local.z),
                radius,
                to: world
                    .inverse()
            ...
            scrap_import::terrain::Edit::Raise {
                at: (local.x, local.z),
                radius,
                by: by / scale.y.max(1e-4),
            }
        };
        let line = scrap::ron::to_string(&edit).map_err(|e| EditError::Scene(e.to_string()))?;
    ...
    pub fn apply_overrides(&mut self, instance: EntityId) -> EditResult<usize> {
        ...
        scrap::Prefabs::save(&prefab, &path).map_err(EditError::Io)?;
    ...
    pub fn make_variant(&mut self, instance: EntityId, name: &str) -> EditResult<()> {
        ...
        let variant = EntityDesc {
            ...
            transform: scrap::Transform::default(),
            ..line.clone()
        }
    ...
    pub fn find_path(
        ...
        from: Vec3,
        to: Vec3,
        settings: scrap::navigation::NavSettings,
    ) -> Option<Vec<Vec3>> {
    ...
    pub(crate) fn bake_navigation(
        ...
        scrap::physics::attach_scene_collision_meshes(&mut world, scene, self.library.as_ref());
        let mut physics = scrap::PhysicsWorld::new(1.0 / 60.0);
        physics.sync_from_world(&mut world);
        physics.refresh_queries();
        ...
        for (placed, body) in world
            .query::<(&scrap::world::WorldTransform, &scrap::world::Physics)>()
            .iter()
        {
            if body.0 != scrap::Body::None {
        ...
        scrap::navigation::NavGrid::bake(
            &physics,
            scrap::glam::Vec2::new(min.x, min.z),
            scrap::glam::Vec2::new(max.x, max.z),
            settings,
        )
    }
    ...
    pub fn scene_at(&self, commit: &str) -> EditResult<Scene> {
        ...
        let parsed = if is_prefab(Some(path)) {
            scrap::ron::from_str::<EntityDesc>(&text).map(|root| Scene {
                entities: vec![root],
                ..Scene::default()
            })
        } else {
            scrap::ron::from_str::<Scene>(&text)
        };
        let mut scene =
    ...
    pub fn environment(&self) -> Vec<(&'static str, String)> {
        ...
        scrap::moods::LOOK_FIELDS
            .iter()
            .map(|field| {
                (
                    *field,
                    scrap::scene::look_field(scene, field).unwrap_or_default(),
                )
            })
            .collect()
    }
    ...
    pub fn set_look(&mut self, changes: &[(&str, &str)]) -> EditResult<()> {
        ...
        for (field, ron) in changes {
            scrap::scene::set_look_field(&mut scene, field, ron).map_err(EditError::Scene)?;
        }
    ...
    /// Put a mood on the scene ([`scrap::moods`]): its fields, as one
    /// undo step.
    pub fn apply_mood(&mut self, name: &str) -> EditResult<()> {
        let mood = scrap::moods::mood(name).ok_or_else(|| {
            EditError::Scene(format!(
    ...
    pub fn project(&self) -> Option<&scrap::Project> {
        self.project.as_ref()
    }
        ...
        for (desc, _) in self.instanced.scene.flatten() {
            for (component, value) in &desc.components {
                for (kind, link) in scrap::refs::links_in(value.get_ron()) {
                    ...
                    let near = scrap::spelling::closest(&link, names.iter().map(String::as_str))
                ...
                let found = match name.strip_prefix("builtin:") {
                    ...
                    None => {
                        library.and_then(|l| l.material_by_name(name)).is_some()
                            || scrap::material::builtin::by_name(name).is_some()
                    }
                };
                if !found {
                    out.push(Diagnostic {
                        ...
                        message: format!(
                            ...
                            suggest(name, &materials, &scrap::material::builtin::NAMES)
                        ),
                    });
                }
            }
    ...
    /// [`scrap::query`]). A query that cannot mean anything says why.
    pub fn search(&self, query: &str) -> EditResult<Vec<EntityId>> {
        let query: scrap::query::Query = query.parse().map_err(EditError::Scene)?;
    ...
    pub fn transform(&self, id: EntityId) -> Option<scrap::Transform> {
        self.line(id).map(|e| e.transform)
    }

    /// Set an entity's local transform, as one undoable step.
    pub fn set_transform(&mut self, id: EntityId, transform: scrap::Transform) -> EditResult<()> {
    ...
    fn modify(&mut self, id: EntityId, change: impl FnOnce(&mut EntityDesc)) -> EditResult<()> {
        ...
        change(&mut edited);
        let delta = scrap::scene::Override::between(&current, &edited);
        if delta.is_empty() {
            return Ok(());
        }
    ...
    pub fn world_position(&self, id: EntityId) -> Option<Vec3> {
        ...
        self.world
            .query::<(&scrap::SceneId, &scrap::world::WorldTransform)>()
            .iter()
    ...
    ///
    /// Laid out by [`scrap::edit::scatter`]: the same seed gives the same
    ...
    pub fn scatter(
        ...
        let children = scrap::edit::scatter(layout)
            ...
            .map(|transform| {
                ...
                .with(scrap::scene::ModelRef(if prefab {
                    Default::default()
                } else {
                    what.into()
                }))
    ...
    pub fn paint_foliage(
        ...
        for _ in 0..tries {
            ...
            let mut transform = scrap::Transform {
                position: point,
                scale: Vec3::splat(0.8 + 0.4 * next()),
                ..Default::default()
            };
            let yaw = scrap::glam::Quat::from_rotation_y(next() * std::f32::consts::TAU);
            let lean = if align {
                scrap::glam::Quat::from_rotation_arc(Vec3::Y, normal.normalize_or_zero())
            } else {
                scrap::glam::Quat::IDENTITY
            };
    ...
    pub fn delete(&mut self, id: EntityId) -> EditResult<()> {
        self.refuse_while_playing()?;
        self.require(id)?;
        scrap::edit::remove(self.history.edit(), id);
        self.after_structural_change();
        Ok(())
    }
    ...
    pub fn duplicate(&mut self, id: EntityId) -> EditResult<EntityId> {
        ...
        self.require(id)?;
        let copy =
            scrap::edit::duplicate(self.history.edit(), id).ok_or(EditError::NoEntity(id))?;
        self.respawn();
        Ok(copy)
    }

    /// `count` copies of an entity in a row, `step` apart, as one undo
    /// step (see [`scrap::edit::array`]). Returns the copies' IDs.
    pub fn array(&mut self, id: EntityId, count: usize, step: Vec3) -> EditResult<Vec<EntityId>> {
        ...
        let copies = scrap::edit::array(self.history.edit(), id, count, step);
        self.respawn();
        Ok(copies)
    }
    ...
    pub fn reparent(&mut self, id: EntityId, new_parent: Option<EntityId>) -> EditResult<bool> {
        ...
        if !scrap::edit::reparent(&mut trial, id, new_parent) {
            return Ok(false);
        }
    ...
    /// scene named by `SCRAP_SCENE`, the file it watches by
    /// `SCRAP_SCENE_FILE`) for the window to start. Run alone the game
    ...
    pub fn move_in_hierarchy(
        ...
        let mut trial = self.history.scene().clone();
        if !scrap::edit::reparent_at(&mut trial, id, new_parent, index) {
            return Ok(false);
        }
    ...
    /// Point an entity at a material by name — a `.scrmat` in the library, or
    /// a builtin.
    ///
    ...
    pub fn palette(&self) -> Vec<(String, Material)> {
        ...
        if let Some(library) = self.library.as_ref() {
            for name in library.names_of(scrap::asset::MATERIAL) {
                if let Some(material) = library.material_by_name(name) {
                    out.push((name.to_string(), material));
                }
            }
        }
        for name in scrap::material::builtin::NAMES {
            ...
            if let Some(material) = scrap::material::builtin::by_name(name) {
                out.push((name.to_string(), material));
            }
        }
        out
    ...
    /// every other scene can use. Writes a `.scrmat` source into `materials/`
    ...
    pub fn save_material(&mut self, id: EntityId, name: &str) -> EditResult<()> {
        ...
        // the editor's floats.
        let channel = |value: f32| (scrap::material::linear_to_srgb(value) * 255.0).round() as u8;
        let text = format!(
            "(color: \"#{:02x}{:02x}{:02x}\"{})\n",
            ...
            match material.shading {
                scrap::Shading::Unlit => ", unlit: true",
                scrap::Shading::Grid => ", grid: true",
                scrap::Shading::Water => ", water: true",
                scrap::Shading::Sand => ", sand: true",
                scrap::Shading::Lit => "",
            }
        );
        std::fs::create_dir_all(&source_dir)?;
        let source = source_dir.join(format!("{name}.scrmat"));
        std::fs::write(&source, text)?;
        match &self.project {
            Some(project) => {
                scrap_import::import_into(project, &source, None)
                    .map_err(|e| EditError::Import(format!("{e:#}")))?;
            }
            None => {
                let settings = scrap_import::ImportSettings::for_source(
                    source.to_string_lossy().into_owned(),
                );
                scrap_import::import_file(&source, &library_dir, settings)
                    .map_err(|e| EditError::Import(format!("{e:#}")))?;
            }
        }
        self.reopen_library()?;
    ...
    pub fn set_prefabs(&mut self, directory: impl AsRef<Path>) -> EditResult<Vec<String>> {
        ...
        let (prefabs, problems) =
            scrap::Prefabs::open(&directory).map_err(|e| EditError::Io(e.to_string()))?;
        self.prefabs = prefabs;
    ...
    pub fn make_prefab(&mut self, id: EntityId, name: &str) -> EditResult<()> {
        ...
        std::fs::create_dir_all(&directory)?;
        let path = directory.join(format!("{name}.{}", scrap::prefab::EXTENSION));
        scrap::Prefabs::save(&desc, &path).map_err(EditError::Io)?;
        ...
        entity.set_part(&scrap::scene::ModelRef(Default::default()));
    ...
    pub fn rename_asset(
        ...
        let renamed = scrap_import::assets::rename(&project, from.as_ref(), to.as_ref())
            .map_err(|e| EditError::Import(format!("{e:#}")))?;
        ...
        if let Some((old, new)) = &renamed.reference {
            ...
            if let Some(path) = &self.scene_path {
                if let Some((seen, stamps)) = &mut self.on_disk {
                    scrap::refs::rewrite_scene(seen, old, new.name());
                    *stamps = scrap::live::stamps(path, Some(&project));
                }
            }
        }
        self.prefabs = scrap::Prefabs::of(&project).0;
    ...
    fn refuse_if_locked_by_others(&self, project: &scrap::Project, file: &Path) -> EditResult<()> {
    ...
    ) -> EditResult<Vec<scrap_import::assets::Usage>> {
        let project = self.project.as_ref().ok_or(EditError::NotInProject)?;
        scrap_import::assets::usages(project, file.as_ref())
            .map_err(|e| EditError::Import(format!("{e:#}")))
    }
    ...
    pub fn delete_asset(&mut self, file: impl AsRef<Path>) -> EditResult<()> {
        ...
        if let Some(what) = scrap_import::assets::reference(&project, file)
            .map_err(|e| EditError::Import(format!("{e:#}")))?
        {
            let unsaved = scrap::refs::uses_in_scene(self.history.scene(), &what);
    ...
    pub fn import(&mut self, source: impl AsRef<Path>) -> EditResult<Vec<String>> {
        ...
        let warnings = match &self.project {
            Some(project) => {
                let report = scrap_import::import_into(project, source, None)
                    .map_err(|e| EditError::Import(format!("{e:#}")))?;
    ...
    /// first — a changed `.png` becomes a changed `.scrasset` — and then the
    ...
    /// Unreal's Material Instance (docs/artist.md): `materials/<parent>_instance.scrmat`
    ...
    pub fn new_material_instance(&mut self, parent: &str) -> EditResult<String> {
        ...
        let mut n = 2;
        while dir.join(format!("{name}.scrmat")).exists() {
            name = format!("{parent}_instance_{n}");
            n += 1;
        }
        let link = match scrap::asset::sidecar_id(scrap::asset::sidecar_of(&source)) {
            Some(id) => format!("({parent:?}, \"{id}\")"),
            None => format!("{parent:?}"),
        };
        std::fs::write(
            dir.join(format!("{name}.scrmat")),
            format!(
    ...
    pub fn assets_of_kind(&self, kind: &str) -> Vec<String> {
        ...
        let mut out: Vec<String> = match kind {
            "model" => {
                let mut v = library(scrap::asset::MESH);
                v.extend(builtin::NAMES.iter().map(|n| n.to_string()));
                v
            }
            "material" => library(scrap::asset::MATERIAL),
            "sound" => library(scrap::asset::SOUND),
            "texture" => library(scrap::asset::TEXTURE),
            "prefab" => self
                .prefabs
                .names()
                .into_iter()
    ...
    pub fn link_exists(&self, kind: &str, link: &scrap::AssetLink) -> bool {
        use scrap::asset::AssetKind;
        ...
        match kind {
            "model" => builtin::by_name(link).is_some() || library(scrap::asset::MESH),
            "material" => library(scrap::asset::MATERIAL),
            "sound" => library(scrap::asset::SOUND),
            "texture" => library(scrap::asset::TEXTURE),
    ...
    pub fn link_to(&self, kind: &str, name: &str) -> scrap::AssetLink {
        let mut link = scrap::AssetLink::named(name);
        let found = match kind {
            "model" => self
                .library
                .as_ref()
                .and_then(|l| l.find(&link, scrap::asset::MESH)),
            "material" => self
                .library
                .as_ref()
                .and_then(|l| l.find(&link, scrap::asset::MATERIAL)),
            "sound" => self
                .library
                .as_ref()
                .and_then(|l| l.find(&link, scrap::asset::SOUND)),
            "texture" => self
                .library
                .as_ref()
                .and_then(|l| l.find(&link, scrap::asset::TEXTURE)),
            _ => None,
        }
    ...
    /// A material's `.scrmat` in the project, by name.
    ...
    pub fn material_layers(&self, name: &str) -> EditResult<scrap_import::MaterialLayers> {
        let file = self.material_file(name)?;
        scrap_import::material_layers(&file).map_err(|e| EditError::Scene(format!("{e:#}")))
    }
    ...
    pub fn set_material_param(
        ...
        if key != "parent" && !known.fields.iter().any(|(k, _, _)| k == key) {
            ...
            let near = scrap::spelling::closest(key, names.iter().copied())
                .map(|n| format!(" — did you mean `{n}`?"))
                .unwrap_or_default();
        ...
        let new = scrap::ron_edit::set_field(&old, key, value).ok_or_else(|| {
            EditError::Scene(format!(
        ...
        if let Err(e) = scrap_import::material_source(&file) {
            ...
            None => {
                ...
                self.syncing = Some((
                    std::thread::spawn(move || {
                        scrap_import::sync_settled(&project, std::time::Duration::from_secs(2))
                    }),
        ...
        for message in messages {
            match message {
                blender_link::Message::Imported { source, result } => {
                    ...
                    self.apply_synced(vec![scrap_import::Reimported {
                        source,
                        change: scrap_import::Change::Changed,
                        result,
                    }]);
                    moved = false;
                }
    ...
    fn apply_synced(&mut self, synced: Vec<scrap_import::Reimported>) -> usize {
    ...
    /// What the scrap panel in Blender offers, written where it reads it,
    ...
    pub fn reload_assets(&mut self) -> usize {
        ...
        if let Some(project) = self.project.clone() {
            ...
            let synced = scrap_import::sync(&project);
    ...
    pub fn capture_camera(&mut self) -> EditResult<()> {
        self.refuse_while_playing()?;
        let view = scrap::captured_view(&self.camera);
    ...
    pub fn render(&mut self) {
        ...
        scrap::soft::step(&mut self.world, 1.0 / 30.0);
        scrap::soft::show(&mut self.world, 0.0);
        scrap::destruction::show(&mut self.world, 0.0);
        // Water, snow and sand move as they are looked at, too.
        scrap::fluid::step(&mut self.world, 1.0 / 30.0);
        scrap::fluid::show(&mut self.world, 0.0);
        scrap::character::crawl(&mut self.world, 1.0 / 30.0);
        scrap::character::show(&mut self.world, 0.0);
        // A clip previewed moves the same way: a thirtieth a frame drawn.
        if !self.previewing.is_empty() && self.play.is_none() {
            scrap::advance_animations(&mut self.world, 1.0 / 30.0);
        }
        ...
        scrap::terrain::upload_terrains(&mut self.world, &self.gpu, &mut self.renderer);
        ...
        let _ = scrap::world::upload_material_maps(
            &self.world,
            self.library.as_ref(),
            &self.gpu,
            &mut self.renderer,
        ...
        if self.show_colliders {
            ...
            for (desc, placed) in self.instanced.scene.flatten() {
                if desc.body() == scrap::Body::None || unseen.contains(&desc.id) {
                    continue;
                }
        ...
        if let Some((settings, grid)) = &self.nav_shown {
            ...
            frame
                .draws
                .extend(grid.strips().into_iter().map(|(at, length)| scrap::Draw {
                    mesh: arm,
                    ...
                    texture: scrap::TextureHandle::WHITE,
        ...
        if !selection.is_empty() {
            ...
            for (desc, placed) in self.instanced.scene.flatten() {
                ...
                if let Some(route) = &desc.route() {
                    ...
                    let way: Vec<Vec3> =
                        scrap::routes::Travelling::new(route.clone(), desc.transform.position)
                            .points()
                            .into_iter()
            ...
            for (ids, color) in [
                ...
                let shapes = scrap::build_frame_where(
                    &self.world,
                    self.camera,
                    Lighting::default(),
                    FogSettings::default(),
                ...
                frame
                    .outline_draws
                    .extend(shapes.draws.into_iter().map(|d| scrap::Draw {
                        material: color,
                        pose: None,
                        ..d
                    }));
    ...
    fn draw_angle(&mut self) {
        ...
        ui.quad(
            scrap::Quad::new(
                ...
                width,
                size * 1.3 + pad * 2.0,
                scrap::glam::Vec4::new(0.1, 0.1, 0.1, 0.8),
            )
            .rounded(4.0 * scale),
        );
        ui.text(
            scrap::TextRun::new(x - pad, y, size, scrap::glam::Vec4::ONE, text)
                .within(width, 0.5),
        );
        let label = self
            .label
            .get_or_insert_with(|| scrap::UiRenderer::new(&self.gpu, &self.target));
    ...
    fn base_frame(&self, camera: Camera) -> Frame {
        ...
        scrap::world::post_volumes(&mut frame, &self.world);
        frame.texture_views = scrap::world::texture_views(&self.world, scene, camera);
        frame
    }

    /// A sound asset by the name scenes use, from the library: what the
    /// Project window plays to preview it.
    pub fn sound(&self, name: &str) -> Option<&scrap::asset::ArchivedSoundAsset> {
    ...
    pub fn camera_of(&self, id: EntityId) -> Option<Camera> {
        let (lens, placed) = self
            ...
            .query::<(
                &scrap::SceneId,
                &scrap::world::CameraLens,
                &scrap::world::WorldTransform,
            )>()
            .iter()
            .find(|(s, _, _)| s.0 == id)
            .map(|(_, l, w)| (l.0, w.0))?;
    ...
    /// The Scene view's graphics preset (`scrap::quality`); `None` draws
    /// the scene as it asks, rays and all.
    pub fn set_quality(&mut self, quality: Option<scrap::quality::Quality>) {
        self.renderer.set_quality(quality);
    }

    pub fn quality(&self) -> Option<scrap::quality::Quality> {
    ...
    /// scene (see [`scrap::world::camera_of`]), `None` when the scene has
    ...
    pub fn game_camera(&self) -> Option<Camera> {
        scrap::world::camera_of(&self.world)
    }
    ...
    pub fn set_show_navigation(&mut self, walker: Option<scrap::navigation::NavSettings>) {
    ...
    pub fn face_under(&self, x: u32, y: u32) -> Option<(EntityId, scrap::edit::Face)> {
    ...
    pub fn face_on_screen(&self, id: EntityId, face: scrap::edit::Face) -> Option<FaceOnScreen> {
        ...
        let (w, h) = self.size();
        let size = scrap::glam::Vec2::new(w as f32, h as f32);
    ...
    pub fn screen_of(&self, point: Vec3) -> Option<(f32, f32)> {
        let (w, h) = self.size();
        self.camera
            .screen_point(point, scrap::glam::Vec2::new(w as f32, h as f32))
            .map(|p| (p.x, p.y))
    }
    ...
    pub fn duplicate_selection(&mut self) -> EditResult<Vec<EntityId>> {
        ...
        let copies: Vec<EntityId> = roots
            .iter()
            .filter_map(|id| scrap::edit::duplicate(scene, *id))
            .collect();
        self.respawn();
    ...
    pub fn copy_selection(&self) -> String {
        ...
        let entities: Vec<EntityDesc> = self
            ...
            .collect();
        let pretty = scrap::ron::ser::PrettyConfig::new().depth_limit(3);
        scrap::ron::ser::to_string_pretty(&entities, pretty).unwrap_or_default()
    }
    ...
    pub fn paste(&mut self, text: &str, parent: Option<EntityId>) -> EditResult<Vec<EntityId>> {
        self.refuse_while_playing()?;
        let mut entities: Vec<EntityDesc> = scrap::ron::from_str(text.trim())
            .or_else(|_| scrap::ron::from_str::<EntityDesc>(text.trim()).map(|one| vec![one]))
        ...
        let pasted: Vec<EntityId> = entities
            .into_iter()
            .filter_map(|desc| scrap::edit::add(scene, parent, desc))
            .collect();
        self.respawn();
    ...
    pub fn gizmo(&self) -> Option<gizmo::Gizmo> {
        // The hand moves the view, not the selection: no handles to grab.
        if self.hand {
            return None;
        }
    ...
    pub fn gizmo_drag(&mut self, x: u32, y: u32) -> EditResult<bool> {
        ...
        let turned = orientation != scrap::glam::Quat::IDENTITY;
        ...
        let started_at = |from: &scrap::Transform, parent: Mat4| parent.transform_point3(from.position);
        let others: Vec<(EntityId, scrap::Transform, Mat4)> = self
            .drag_others
            .iter()
    ...
    pub fn play(&mut self) {
        ...
        let fixed = scrap::TimeSettings::default().fixed_delta;
        let mut physics = scrap::PhysicsWorld::new(fixed);
        ...
        // already applied.
        scrap::physics::attach_scene_collision_meshes(
            &mut self.world,
    ...
    pub fn step(&mut self, seconds: f32) -> u32 {
        ...
        // solver does.
        scrap::advance_animations(&mut self.world, seconds);
        steps
    }
    ...
    fn fixed_step(world: &mut hecs::World, physics: &mut scrap::PhysicsWorld, fixed: f32) {
        ...
        scrap::destruction::step(world, physics, fixed);
    }
    ...
    pub fn stop(&mut self) -> bool {
        ...
        let kept: Vec<(EntityId, scrap::Transform)> = self
            .world
            .query::<(&scrap::world::SceneId, &scrap::Transform)>()
            .iter()
    ...
    fn insert(&mut self, parent: Option<EntityId>, desc: EntityDesc) -> EditResult<EntityId> {
        ...
        let id = scrap::edit::add(self.history.edit(), parent, desc)
    ...
    fn respawn(&mut self) {
        ...
        // about prefabs.
        self.instanced = scrap::instantiate(self.history.scene(), &self.prefabs);
    ...
    fn ray(&self, x: u32, y: u32) -> (Vec3, Vec3) {
        ...
        self.camera.ray_through(
            scrap::glam::Vec2::new(x as f32 + 0.5, y as f32 + 0.5),
            scrap::glam::Vec2::new(w as f32, h as f32),
        )
    }
...
fn save_document(scene: &Scene, path: &Path) -> EditResult<()> {
    if is_prefab(Some(path)) {
        ...
        return scrap::Prefabs::save(root, path).map_err(EditError::Io);
    }
    scene
        .save(path)

...
use scrap::input::{Input, Key, MouseButton};
    ...
    fn scene_view_frame(&mut self, input: &Input, dt: f32) -> EditResult<Vec<&'static str>> {
        ...
        if input.mouse_pressed(MouseButton::Left) && vertex && self.vertex_begin(x, y)? {
            ...
            if input.mouse_held(MouseButton::Left) {
                if motion != scrap::glam::Vec2::ZERO && self.vertex_drag(x, y)? {
            ...
            if input.mouse_held(MouseButton::Left) && !alt && motion != scrap::glam::Vec2::ZERO {
            ...
            && motion != scrap::glam::Vec2::ZERO
        {
            ...
            let middle = held == Some(scrap::gizmo::Grip::new(Tool::Move, scrap::gizmo::Handle::Center));
        ...
        if alt && input.mouse_held(MouseButton::Left) && motion != scrap::glam::Vec2::ZERO {
        ...
        if input.mouse_held(MouseButton::Middle) && motion != scrap::glam::Vec2::ZERO {
            let camera = self.camera();
            let scale = (camera.position - camera.target).length() * 0.0015;
            self.pan(-motion.x * scale, motion.y * scale);
            did.push("pan");

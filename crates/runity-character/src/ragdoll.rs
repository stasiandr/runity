...
use runity_core::world::{SceneId, WorldTransform};
use runity_core::{EntityId, EntityRef};
use runity_physics::bodies::{Jointed, Physics, Props, Shape};
use runity_physics::body::{Body, BodyProps, Collider, Joint};
...
impl RagdollLine for runity_core::EntityDesc {
    fn ragdoll(&self) -> Option<Ragdoll> {
        self.part()
    }
}

impl RagdollLine for runity_core::scene::Override {
    fn ragdoll(&self) -> Option<Ragdoll> {
        self.part()
    }
}
...
impl runity_core::world::Dress for RagdollDress {
...
impl runity_core::world::Dress for CrawlerDress {
    fn parts(&self) -> &[&'static str] {
        &["crawler"]
    }

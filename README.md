...
ряд), 1–4 и N — отладочные виды (на GPU 2 и 4 отказывают и на две секунды
говорят почему в заголовке окна), H — список клавиш поверх затемнённой сцены,
F — HUD целиком, Escape — выход. Каждая сцена помнит свою камеру и замирает,
пока не на экране; отладочный вид общий и переживает переключение. Панель в
левом верхнем углу — номер и имя сцены, fps, отладочный вид, число
треугольников и фрагментов последнего кадра — рисуется в кадре, а не в
заголовке окна: она попадает в скриншот и в headless-эталон так же, как всё
остальное. `RUNITY_SCENE=1..7` открывает нужную сцену сразу,
`RUNITY_HEADLESS=1` прогоняет все семь по 60 кадров с шагом 1/60 и раскладывает
их по PNG — без HUD, только сцена. Полный список клавиш и переменных — в
док-комментарии `crates/runity/examples/smallworld.rs`.
...
что единственное, что лежит в `Contents/Resources` — скомпилированная
шейдерная библиотека). Бандл идёт на GPU: у двойного щелчка нет переменных
окружения, а окно по умолчанию и так Metal. Скрипт заодно компилирует
`shader.metal` через `xcrun metal` и кладёт `runity.metallib` в
...
сцены, Backspace — отменить его, H — список клавиш, F — HUD целиком, Escape —
выход. Headless-PNG бандл не пишет —

...
последние десять), 1–4 и N — отладочные виды, H — список клавиш поверх
затемнённой сцены, F — HUD целиком, Escape — выход. Каждая сцена помнит свою
камеру и замирает, пока не на экране; отладочный вид общий и переживает
переключение. Панель в левом верхнем углу — номер и имя сцены, fps,
отладочный вид, число треугольников и фрагментов последнего кадра —
рисуется в кадре, а не в заголовке окна: она попадает в скриншот и в
headless-эталон так же, как всё остальное. `RUNITY_SCENE=1..6` открывает
нужную сцену сразу, `RUNITY_HEADLESS=1` прогоняет все шесть по 60 кадров с
шагом 1/60 и раскладывает их по PNG — без HUD, только сцена. Полный список
клавиш — в док-комментарии `crates/runity/examples/smallworld.rs`.
...
переменных окружения (модель сцен вшита в бинарник через `include_str!`, так
что `Contents/Resources` пустой, а бандл подписан ad-hoc, чтобы Gatekeeper не
ругался на локальной машине). Управление то же, что и у примера: Tab /
Shift+Tab — сцены по кругу, стрелки или WASD — камера, Q/E — зум, 1–4 и N —
отладочные виды, пробел — действие сцены, Backspace — отменить его, H —
список клавиш, F — HUD целиком, Escape — выход. Headless-PNG бандл не пишет —

...
| Рендер | `runity-render` | отложенный растеризатор с PBR, IBL, тенями, SSAO, SSR, bloom и оптикой; мип-маппинг, отсечение по пирамиде видимости, debug-виды и текст в кадре, golden-тесты, PNG + DEFLATE, JSON и glTF 2.0 (включая GLB) |
...
вывод звука в систему (CoreAudio/ALSA/WASAPI), анимация и скиннинг (glTF читается,
но без них), HiDPI на macOS, сжимающий PNG-энкодер; из симуляции — сглаживание коррекций на клиенте
